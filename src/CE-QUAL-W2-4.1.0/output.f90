...
!********* macrophytes
      do L = 1, nmc
         ...
         IF (PRINT_macrophyte(jw, L)) THEN
            ...
            NLINES = NLINES + KMX - KTWB(JW) + 11
            NEW_PAGE = NLINES > 72
        WRITE (SNP(jw),'(/1X,3(A,1X,I0),A,F0.2,2A,i0,A/)')  MONTH,GDAY,',',YEAR,'    Julian Date ',INT(JDAY),' days ', &
        (JDAY-INT(JDAY)) * 24.0, ' hours   ', '  Macrophyte Group #', L, ' g/m^3'  ! cb 12/20/11
            WRITE (SNP(JW), '(1X,A,1000I10)') 'Layer  Depth', (ISNP(I, JW), I=IUPR, IDPR)
            DO K = ktwb(jw), KBR
               WRITE (SNP(JW), '(1X,I4,F8.2,1000A10)') K, DEPTHM(K, DS(BS(JW))), (CONV(K, I), I=IUPR, IDPR)
            END DO
...
!                                                     *24.0,' hours   ','  Macrophyte Columns g/m^3'
          WRITE (SNP(JW),'(/1X,3(A,1X,I0),A,F0.2,2A,i0,A/)')  MONTH,GDAY,',',YEAR,'    Julian Date ',INT(JDAY),' days ', &
          (JDAY-INT(JDAY)) * 24.0, ' hours   ', '  Macrophyte Group #', L, ' Columns g/m^3'! cb 12/20/11
