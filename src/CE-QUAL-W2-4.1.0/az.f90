...
RECURSIVE SUBROUTINE CALCULATE_AZ
   ...
   REAL(R8)            :: TKEMIN1, TKEMIN2, RIAZ0, AZ0, RIAZ1, EXPAZ, DEPTHL, DEPTHR, ZDLR, DEPTH, USTAR, ZD
...
!CALCULATE HORIZONTAL PORTION OF THE SPLIT K-E EQUATIONS FOR INTERIOR LAYERS
   DO K = KT + 1, KB(I) - 1
      ...
      CONVKX = DLT/(DLX(I)*BH1(K, I))*((U(K, I)*(UDR*TKE(K, I, 1) + (1.0 - UDR)*TKE(K, I + 1, 1))*BR(K, I)*H2(K, I) - U(K, I - 1)* &
                                        (UDL*TKE(K, I - 1, 1) + (1.0 - UDL)*TKE(K, I, 1))*BR(K, I - 1)*H2(K, I - 1)))
      CONVEX = DLT/(DLX(I)*BH1(K, I))*((U(K, I)*(UDR*TKE(K, I, 2) + (1.0 - UDR)*TKE(K, I + 1, 2))*BR(K, I)*H2(K, I) - U(K, I - 1)* &
                                        (UDL*TKE(K, I - 1, 2) + (1.0 - UDL)*TKE(K, I, 2))*BR(K, I - 1)*H2(K, I - 1)))
      ...
      ELSE
         ...
         AB = (1.0D0 + DSIGN(1.0D0, W(K, I)))*0.5D0
                 ...
                 CONVKZ = DLT/(H2(K,I)*BH1(K,I))*((AB*TKE(K,I,1)+(1.0-AB)*TKE(K+1,I,1))* &
                 W(K,I)*BB(K,I)*AVH2(K,I)-(ATA*(TKE(K-1,I,1)+(1.0-ATA)*TKE(K,I,1))*W(K-1,I)*BB(K-1,I)*AVH2(K-1,I)))
                 CONVEZ = DLT/(H2(K,I)*BH1(K,I))*((AB*TKE(K,I,2)+(1.0-AB)*TKE(K+1,I,2))* &
                 W(K,I)*BB(K,I)*AVH2(K,I)-(ATA*(TKE(K-1,I,2)+(1.0-ATA)*TKE(K,I,2))*W(K-1,I)*BB(K-1,I)*AVH2(K-1,I)))
