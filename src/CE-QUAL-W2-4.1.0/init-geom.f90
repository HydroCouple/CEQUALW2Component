...
! Constrictions                       ! SW 6/26/2018
   ...
   BCONSTRICTION = 0.0
   INQUIRE (FILE='constriction.csv', EXIST=Constriction(1, 1))     !  Just use the first element for file existance
   IF (CONSTRICTION(1, 1)) THEN
      OPEN (CON, FILE='constriction.csv', STATUS='OLD')
      READ (CON, *)
      READ (CON, *) ICON    ! NUMBER OF CONSTRICTIONS
      READ (CON, *)
      DO J = 1, ICON
