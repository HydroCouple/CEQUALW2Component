               ...
               IF (K.LT.KTWB(WB)) THEN                               ! WATER used for value-blanking cells above 9952 surface in TecPlot
                 IF (SHOWSKY) THEN                                  ! Show Colors of Day Above Water Surface
                   ...
                   ELSE IF ((MILTIME.GT.SKYDUSK).AND.&
                     ...
                     WATER = -25                                    !   ORANGE
                   ELSE                                             ! CHECK
                     WRITE(DATADEBUGFN,*) 'ERROR: MILTIME not Corresponding with Intervals &
                     Set for Determining Sky Color for TecPlot'
                     STOP
                   END IF
                 ELSE
...
! IFISHB: Bottommost Layer where Fish are added in Segment IFISH
 ...
 IF(PARTICLE)THEN
     READ(DIAGFN,1001)FXLOC,FZLOC,OUTFREQ
 ELSE
...
!        HAOPERAT(SND,14) = 6              ! Branch # Where Hydroacoustic Sampling is Done
 ...
 READ(DIAGFN,1001)OUTFREQJAN,OUTFREQFEB,OUTFREQMAR,OUTFREQAPR,OUTFREQMAY,&
 OUTFREQJUN,OUTFREQJUL,OUTFREQAUG,OUTFREQSEP,OUTFREQOCT,OUTFREQNOV,OUTFREQDEC
