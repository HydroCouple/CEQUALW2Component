...
!    Common /CEMACmBl1/ CEMAShieldsNumber, SScour, SNetFlx
   ...
   Do JW = 1, NWB
      ...
      Do JB = BS(JW), BE(JW)
         ...
         ID = DS(JB)
         Do SegNumI = IU, ID
            TotalPoreWatVolume = TotalPoreWatVolume + &
                                 BedPorosity(SegNumI)*BedElevation(SegNumI)*B(KB(SegNumI), SegNumI)*DLX(SegNumI)
            TotalSedimentsInBed = TotalSedimentsInBed + &
                                  (1 - BedPorosity(SegNumI))*BedElevation(SegNumI)*B(KB(SegNumI), SegNumI)*DLX(SegNumI)
