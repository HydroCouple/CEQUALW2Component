...
101     CONTINUE  
        ...
        READ (SPRV(JW),*)  
        DO WHILE (JDAY1 < JDAY)  
          READ (SPRV(JW),'(A,F10.0)',END=104) LINE(1:38),JDAY1  
        END DO  
        BACKSPACE (SPRV(JW))  
104     CONTINUE  
        JDAY1 = 0.0   
...
!104   CONTINUE  
    ...
    OPEN (VPL(1),FILE=VPLFN(1),STATUS='UNKNOWN',ACCESS='STREAM',FORM='UNFORMATTED',POSITION='APPEND')  
  ...
  ELSE  
    ...
    DO JW=1,NWB  
      ...
      IF (PROFILE(JW).and. iprf(1,1) /= -1) THEN    ! SW 4/1/2016
        ...
        WRITE (PRF(JW),'(2A)')       'Temperature, �C                            ',ADJUSTL(CNAME),ADJUSTL(CDNAME)  
        ...
        DO JC=1,NAC  
          IF (PRINT_CONST(CN(JC),JW)) THEN  
            DO JP=1,NIPRF(JW)  
              NRS = KB(IPRF(JP,JW))-KTWB(JW)+1  
              WRITE (PRF(JW),'(A,I4/(8(E13.6,1x)))') ADJUSTL(CNAME2(CN(JC))), &
              NRS,(C2(K,IPRF(JP,JW),CN(JC))*CMULT(CN(JC)),K=KTWB(JW),  &                    ! CB 1/25/05  
              KB(IPRF(JP,JW)))  
            END DO  
          END IF  
        END DO  
        DO JD=1,NACD(JW)  
          DO JP=1,NIPRF(JW)  
            NRS = KB(IPRF(JP,JW))-KTWB(JW)+1  
            WRITE (PRF(JW),'(A,I4/(8(E13.6,1x)))') ADJUSTL(CDNAME2(CDN(JD,JW))), &
            NRS,(CD(K,IPRF(JP,JW),CDN(JD,JW))*CDMULT(CDN(JD,JW)), &                     ! CB 1/25/05  
      ...
      IF (SPREADSHEET(JW)) THEN  
        ...
        END DO  
        IF(SPRC(JW)   == '     ONV')THEN
            WRITE (SPRV(JW),'(A,A,1000(A7,","))') 'Constituent,','Julian_day,',(SEG(J),J=1,NISPR(JW))     ! SW 9/28/2018
        ENDIF
      ...
      IF (CONTOUR(JW)) THEN  
        ...
        19233 FORMAT('VARIABLES="Distance, m","Elevation, m","U(m/s)","W(m/s)","T(C)","RHO", "HABITAT" ', &
        "<NAC>"(',"',A8,'"'),"<NACD(JW)>"(',"',A8,'"'))
        19234 FORMAT('VARIABLES="Distance, m","Elevation, m","U(m/s)","W(m/s)","T(C)","RHO" ',"<NAC>"(',"',A8,'"'))  ! SW 9/28/13
        19235 FORMAT('VARIABLES="Distance, m","Elevation, m","U(m/s)","W(m/s)","T(C)","RHO" ',"<NAC>"(',"',A8,'"'), &
        "<NACD(JW)>"(',"',A8,'"'))  ! SW 9/28/13
        ENDIF  
      END IF  
      ...
      DO J=1,NIKTSR  
        ...
        IF (ICE_COMPUTATION) THEN  
          IF(SEDIMENT_CALC(JW))THEN  
            ...
            (CDNAME2(CDN(JD,JW)),JD=1,NACD(JW)),(KFNAME2(KFCN(JF,JW)),JF=1,NAF(JW)),('APLIM',JA=1,NAL), &
            ('ANLIM',JA=1,NAL),('ALLIM',JA=1,NAL)  
          ELSE  
            WRITE (TSR(J),'(1000(A,","))') 'JDAY','DLT(s)','ELWS(m)','T2(C)','U(ms-1)','Q(m3s-1)','SRON(Wm-2)','EXT(m-1)',   &  
            'DEPTH(m)','WIDTH(m)','SHADE','ICETH(m)','Tvolavg(C)','NetRad(Wm-2)','SWSolar(Wm-2)','LWRad(Wm-2)','BackRad(Wm-2)', &
            'EvapF(Wm-2)','ConducF(Wm-2)',  (CNAME2(CN(JC)),JC=1,NAC),                     &  
            ('     EPI',JE=1,NEP),('     MAC',JM=1,NMC),(CDNAME2(CDN(JD,JW)),JD=1,NACD(JW)),(KFNAME2(KFCN(JF,JW)),JF=1,NAF(JW)), &
            ('APLIM',JA=1,NAL),('ANLIM',JA=1,NAL),('ALLIM',JA=1,NAL)    ! SW 10/20/15  
          END IF  
        ELSE  
          IF(SEDIMENT_CALC(JW))THEN  !mlm 7/25/06
            WRITE (TSR(J),'(1000(A,","))')'JDAY','DLT(s)','ELWS(m)','T2(C)','U(ms-1)','Q(m3s-1)','SRON(Wm-2)','EXT(m-1)',   &  
            'DEPTH(m)','WIDTH(m)','SHADE','Tvolavg(C)','NetRad(Wm-2)','SWSolar(Wm-2)','LWRad(Wm-2)','BackRad(Wm-2)','EvapF(Wm-2)', &
            'ConducF(Wm-2)',   &
            (CNAME2(CN(JC)),JC=1,NAC),                     &  
            ('     EPI',JE=1,NEP),('     MAC',JM=1,NMC),'     SED','    SEDP','    SEDN','    SEDC',   &  
            (CDNAME2(CDN(JD,JW)),JD=1,NACD(JW)),(KFNAME2(KFCN(JF,JW)),JF=1,NAF(JW)),('APLIM',JA=1,NAL),('ANLIM',JA=1,NAL), &
            ('ALLIM',JA=1,NAL)   
          ELSE  
            WRITE (TSR(J),'(1000(A,","))') 'JDAY','DLT(s)','ELWS(m)','T2(C)','U(ms-1)','Q(m3s-1)','SRON(Wm-2)','EXT(m-1)',   &  
             'DEPTH(m)','WIDTH(m)','SHADE','Tvolavg(C)','NetRad(Wm-2)','SWSolar(Wm-2)','LWRad(Wm-2)','BackRad(Wm-2)', &
             'EvapF(Wm-2)','ConducF(Wm-2)',   &
            (CNAME2(CN(JC)),JC=1,NAC),                     &  
            ('     EPI',JE=1,NEP),('     MAC',JM=1,NMC),(CDNAME2(CDN(JD,JW)),JD=1,NACD(JW)),(KFNAME2(KFCN(JF,JW)),JF=1,NAF(JW)), &
            ('APLIM',JA=1,NAL),('ANLIM',JA=1,NAL),('ALLIM',JA=1,NAL)   
          END IF  
        END IF  
    ...
    IF (DOWNSTREAM_OUTFLOW) THEN  
      ...
      DO JWD=1,NIWDO  
        ...
        WRITE (WDO(JWD,1),'(A,I0/A/A)') '$Flow file for segment ',       IWDO(JWD), &
        'To the right of the sum of flows are individual flows starting with QWD then QSTR','JDAY,QWD(m3s-1),'  
        WRITE (WDO(JWD,2),'(A,I0/A/A)') '$Temperature file for segment ',IWDO(JWD), &
        'To the right of the sum of temperatures are individual temperatures starting with QWD then QSTR','JDAY,T(C),'  
        DO JW=1,NWB  
        ...
        IF (DERIVED_CALC) THEN  
          ...
          WRITE (WDO(JWD,4),'(A,I0//(100(A,",")))') 'Derived constituent file for segment ',IWDO(JWD),'JDAY', &
          (CDNAME2(CDN(J,JW)),J=1,NACD(JW))                 !CDNAME2(CDN(1:NACD(JW),JW))  
        END IF  
      END DO  
    END IF  
    ...
    DO JW=1,NWB  
      IF(VOLUME_BALANCE(JW).AND.CONTOUR(JW))THEN  
        ...
        IF(VOLUME_BALANCE(JW))THEN
            WRITE(FLOWBFN,'(a120)') &
            'JDAY,WB,VOLIN(m3),VOLPR(m3),VOLOUT(m3),VOLWD(m3),VOLEV(m3),VOLDT(m3),VOLTRB(m3),VOLICE(m3),%VOLerror'
        ELSE
    ...
    ENDDO  
      ...
      IF(MASS_BALANCE(JW).AND.CONTOUR(JW).AND.DERIVED_CALC)THEN  
        ...
        IF(SEDIMENT_DIAGENESIS)THEN
            WRITE(MASSBFN,'(A480)')'JDAY,WB,TP-Waterbody(kg),TP-Sediment(kg),TP-Plants(kg),OutflowTP(kg), &
            &TributaryTP(kg),DistributedTributaryTP(kg),WithdrawalTP(kg),PrecipitationTP(kg),InflowTP(kg), &
            &SED+SOD_PRelease(kg),PFluxtoSediments(kg),SedimentDiagenesisPFlux(kg),TN-Waterbody(kg), &
            &TN-Sediment(kg),TN-Plants(kg),OutflowTN(kg),TributaryTN(kg),DistributedTributaryTN(kg), &
            &WithdrawalTN(kg),PrecipitationTN(kg),InflowTN(kg),SED+SOD_NRelease(kg),NFluxtoSediments(kg), &
            &SedimentDiagenesisNH4Flux(kg),SedimentDiagenesisNO3Flux(kg)'  
            
            
            ! 'JDAY,WB,TP-Waterbody(kg),TP-Sediment(kg),TP-Plants(kg),OutflowTP(kg),TributaryTP(kg),DistributedTributaryTP(kg),WithdrawalTP(kg),PrecipitationTP(kg),InflowTP(kg),BurialP_SED(kg),SED+SOD_PRelease(kg),TN-Waterbody(kg),TN-Sediment(kg),TN-Plants(kg),OutflowTN(kg),TributaryTN(kg),DistributedTributaryTN(kg),WithdrawalTN(kg),PrecipitationTN(kg),InflowTN(kg),BurialN_SED(kg),SED+SOD_NRelease(kg)'
            ELSE
            WRITE(MASSBFN,'(A390)')'JDAY,WB,TP-Waterbody(kg),TP-Sediment(kg),TP-Plants(kg),OutflowTP(kg),&
            &TributaryTP(kg),DistributedTributaryTP(kg),WithdrawalTP(kg),PrecipitationTP(kg),InflowTP(kg),&
            &SED+SOD_PRelease(kg),PFluxtoSediments(kg),TN-Waterbody(kg),&
            TN-Sediment(kg),TN-Plants(kg),OutflowTN(kg),TributaryTN(kg),&
            &DistributedTributaryTN(kg),WithdrawalTN(kg),PrecipitationTN(kg),InflowTN(kg),SED+SOD_NRelease(kg),NFluxtoSediments(kg)'   ! 'JDAY,WB,TP-Waterbody(kg),TP-Sediment(kg),TP-Plants(kg),OutflowTP(kg),TributaryTP(kg),DistributedTributaryTP(kg),WithdrawalTP(kg),PrecipitationTP(kg),InflowTP(kg),BurialP_SED(kg),SED+SOD_PRelease(kg),TN-Waterbody(kg),TN-Sediment(kg),TN-Plants(kg),OutflowTN(kg),TributaryTN(kg),DistributedTributaryTN(kg),WithdrawalTN(kg),PrecipitationTN(kg),InflowTN(kg),BurialN_SED(kg),SED+SOD_NRelease(kg)'
        ENDIF
    ...
    IF (VECTOR(1)) THEN  
      ...
      WRITE(VPL(1)) INT(NWB, 4), INT(NBR, 4), INT(IMX, 4), INT(KMX, 4), INT(NCT, 4), INT(NAC, 4)  
      ...
      ! *** MODEL CONFIGURATION  
      DO JW=1,NWB  
        WRITE(VPL(1)) INT(BS(JW), 4),INT(BE(JW), 4)  
      END DO  
        
      WRITE(VPL(1)) (REAL(DLX(I),4) ,I=1,IMX)  
      WRITE(VPL(1)) (REAL(PHI0(I),4),I=1,IMX)  
      WRITE(VPL(1)) (INT(US(K),4) ,K=1,NBR)  
      WRITE(VPL(1)) (INT(DS(K),4) ,K=1,NBR)  
      WRITE(VPL(1)) (INT(UHS(K),4),K=1,NBR)  
      WRITE(VPL(1)) (INT(DHS(K),4),K=1,NBR)  
    ...
    !      END DO  
      ...
      WRITE (VPL(1)) (INT(CN(JC),4),JC=1,NAC)  
