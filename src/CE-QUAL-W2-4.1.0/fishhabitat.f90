SUBROUTINE FISHHABITAT(IOPENFISH)
   USE GLOBAL; USE MAIN; USE SCREENC; USE KINETIC, ONLY: O2, CHLA, NO3, NH4, PO4, TP, GAMMA, SED, SATO
    USE TVDC, ONLY: CONSTITUENTS; USE NAMESC, ONLY: CNAME2, CDNAME2; USE LOGICC
   IMPLICIT NONE
...
REAL, ALLOCATABLE, DIMENSION(:)   :: FISHTEMPL,FISHTEMPH,FISHDO,HABVOL,PHABVOL,CDO,CPO4,CNO3,CNH4,CCHLA,CTOTP,CDOS,CPO4S, &
CNO3S,CNH4S,CCHLAS,CTOTPS,CGAMMA,SSEDD,VOLTOTBR,VOLTOTWB
...
! skip 1st 2 lines
         ...
         allocate (fishname(ifish),fishtempl(ifish),fishtemph(ifish),fishdo(ifish),habvol(ifish), &
         phabvol(ifish),habvolbr(nbr,ifish),habvolwb(nwb,ifish),phabvolbr(nbr,ifish), &
         phabvolwb(nwb,ifish),voltotbr(nbr),voltotwb(nwb))
         read (FISHHABFN, *)
         do i = 1, ifish
         ...
         read (FISHHABFN, *)
         allocate(isegvol(nseg),cdo(nseg),cpo4(nseg),cno3(nseg),cnh4(nseg),cchla(nseg),ctotp(nseg),cdos(nseg), &
         cpo4s(nseg),cno3s(nseg),cnh4s(nseg),cchlas(nseg),ctotps(nseg),cgamma(nseg))
         allocate (ssedd(imx))
...
100         format('JDAY,', "< ifish >" ('%VOL-', A, ',', 'HAB-VOL(m3)-', A, ','))
            ...
            if (oxygen_demand) then
               ...
               write(FISHHABFN+1,101)(trim(cname2(NPO4)),isegvol(i),trim(cname2(NNH4)),isegvol(i), &
               trim(cname2(NNO3)),isegvol(i),trim(cname2(NDO)),isegvol(i),trim(cdname2(12)),isegvol(i), &
               trim(cdname2(14)),isegvol(i),i=1,nseg)   ! Chlor a and TP 
101            format('JDAY,', "< nseg >" (6((A, '-', i3, ','))))
102            format('JDAY,', "< nseg >" (7((A, '-', i3, ','))))

               open (FISHHABFN + 2, file=consurf, status='unknown')
               write(FISHHABFN+2,'(a,i4,a,80(1x,i4))')'Surface (upper',kseg, &
               ' model layers) Volume weighted WQ parameters at segments:',(isegvol(i),i=1,nseg)
               write(FISHHABFN+2,102)(trim(cname2(NPO4)),isegvol(i),trim(cname2(NNH4)), &
               isegvol(i),trim(cname2(NNO3)),isegvol(i),trim(cname2(NDO)),isegvol(i), &
               trim(cdname2(12)),isegvol(i),trim(cdname2(14)),isegvol(i),'Gamma(m-1)',isegvol(i),i=1,nseg)
...
write (FISHHABFN + 3, "(f10.3,',',1000(e13.4,','))") jday, (((ssedd(i), i=us(jb), ds(jb)), jb=bs(jw), be(jw)), jw=1, nwb)    ! OUTPUT IS IN GRAMS
      ...
      deallocate(isegvol,cdo,cpo4,cno3,cnh4,cchla,ctotp,cdos,cpo4s,cno3s,cnh4s,cchlas,ctotps,cgamma,ssedd,fishname,fishtempl, &
      fishtemph,fishdo,habvol,phabvol,habvolbr,habvolwb,phabvolbr,phabvolwb,voltotbr,voltotwb)
      close (FISHHABFN)
      close (FISHHABFN + 1)
      CLOSE (FISHHABFN + 2)
      CLOSE (FISHHABFN + 3)
