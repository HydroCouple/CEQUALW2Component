...
SUBROUTINE KINETICS
  ...
  USE MACROPHYTEC; USE ZOOPLANKTONC; USE MAIN, ONLY:NGCTDG, EPIPHYTON_CALC, BOD_CALC, &
  ALG_CALC, BOD_CALCN, BOD_CALCP, PO4_CALC, N_CALC, DSI_CALC, SEDCOMP_EXIST, JG_AGE, WATER_AGE_ACTIVE
  !SP CEMA
  Use CEMAVars
  !End SP CEMA
...
!!$OMP PARALLEL DO
  DO I=IU,ID
    DO K=KT,KB(I)
      ...
      DO3(K,I)          = (1.0+SIGN(1.0d+0,O2(K,I)-1.E-10)) *0.5
...
! Zooplankton Rates
   IF(ZOOPLANKTON_CALC)THEN
      ...
      END DO
        ...
        END DO
        ZMINFAC  = (1.0+SIGN(1.0d+0,ZOO(K,I,JZ)-ZOOMIN(JZ)))*0.5
...
! CEMA start
             if(jg == ngh2s)then
               ...
               CGSS(K,I,JG) = -CG0DK(JG)*CGQ10(JG)**(T1(K,I)-20.0)-&
               CG1DK(JG)*CGQ10(JG)**(T1(K,I)-20.0)*CG(K,I,JG)-CGLDK(JG)*LIGHT*CG(K,I,JG)+xx
             end if
! CEMA end
         ELSE
...
! CEMA end
    ...
    DOSS(K,I)  =  DOAP(K,I)+DOEP(K,I)-DOAR(K,I)-DOER(K,I)-DOOM(K,I)-DONIT(K,I)-&
    DOSOD(K,I)-DOSED(K,I)+DOMP(K,I)-DOMR(K,I)-DOZR(K,I)+doch4(k,i)+doh2s(k,i)+dofe2(k,i)+doMn2(k,i)  !&     ! CEMA
