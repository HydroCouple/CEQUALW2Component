...
DO JW=1,NWB
      ...
      IF (.NOT. NO_HEAT(JW)) THEN
        ...
        IF (TERM_BY_TERM(JW))THEN                                      ! SW 1/25/05
           ...
           RANLW(JW) = 5.62D-8*(273.15D0+TAIR(JW))**4*&
           (1.D0-0.261D0*DEXP(-7.77D-4*TAIR(JW)*TAIR(JW)))*(1.0D0+0.0017D0*CLOUD(JW)*CLOUD(JW))*0.97D0     ! SW 4/20/16 SPEED
           ENDIF
        ENDIF
      END IF
      DO JB=BS(JW),BE(JW)
...
!************** Ice balance
                ...
                IF (ICE(I)) THEN
                  ...
                  J    = 1
                    ...
                    RANLW(JW) = 5.31D-13*(273.15D0+TAIR(JW))**6*(1.0D0+0.0017D0*CLOUD(JW)**2)*0.97D0
                    ELSE
                    RANLW(JW) = 5.62D-8*(273.15D0+TAIR(JW))**4*(1.D0-0.261D0*&
                    DEXP(-7.77D-4*TAIR(JW)**2))*(1.0D0+0.0017D0*CLOUD(JW)**2)*0.97D0
                    ENDIF
