...
module CE_QUAL_W2_Model
    ...
    ! INTEGER       :: RESULT
    REAL          :: DEPTH
    LOGICAL       :: RESTART_PUSHED = .FALSE., STOP_PUSHED = .FALSE.
...
contains
    subroutine CE_QUAL_W2_GetArray(key, arrayPointer) bind(c, name="CE_QUAL_W2_GetArray")
        ...
        if (key .eq. 0) then 
            arrayPointer = c_loc(NSTR(1))
        else if (key .eq. 1) then 
            arrayPointer = c_loc(QSTR(1,1))
        else if (key .eq. 2) then 
            arrayPointer = c_loc(tavg(1,1))
        else if (key .eq. 3) then 
            arrayPointer = c_loc(QIND(1))
        else if (key .eq. 4) then 
            arrayPointer = c_loc(TIND(1))
        end if
    ...
    integer(kind=c_int) function CE_QUAL_W2_Initialize(dirc) bind(C, name='CE_QUAL_W2_Initialize')
        ...
        character(kind=c_char), DIMENSION(255), intent(in) :: dirc
        CHARACTER(len=:), ALLOCATABLE :: path

        ! include "omp_lib.h"      ! OPENMP directive to adjust the # of processors TOGGLE FOR DEBUG
        path = c_to_f_string(dirc)
        path = trim(path)

        CE_QUAL_W2_Initialize = 0
        ...
        IF (END_RUN .or. ERROR_OPEN) then
            CE_QUAL_W2_Initialize = 1
            return
        end if ! SW 6/26/15 3/18/16 Added code to prevent a thread from reinitializing output files as dialog box is closing...intermittant error Updated 8/23/2017
        ...
        IF (RESTART_IN) THEN
            ...
            READ (RSI) BR_INACTIVE                ! SW 8/1/2018

            if (envirpc == '      ON') THEN

            allocate (cc_e(NCT), C_INT_V(NCT), c_top(NCT), cd_e(NDC), &
                        cd_int(NDC), cd_top(NDC), c_avg(NCT), cd_avg(NDC), cn_e(NCT), cdn_e(NDC))
            ...
            READ (CONE, '(//I1,7X,I8,5x,a3,f8.0,f8.0,9(i8,i8))') I_SEGINT, numclass, selectivec, &
                sjday1, sjday2, istart(1), iend(1), (istart(I), iend(I), I=2, I_SEGINT)
            IF (I_SEGINT == 0) I_SEGINT = 1
            Read (CONE, '(//8x,3(5x,a3,f8.3,f8.3))') VEL_VPR, VEL_INT, VEL_TOP, TEMP_VPR, &
                TEMP_INT, TEMP_TOP, depth_vpr, d_int, d_top
            READ (CONE, '(//(8X,(5X,A3,F8.0,F8.0)))') (CC_E(JC), C_INT_V(JC), C_TOP(JC), JC=1, NCT)
            ...
            allocate (c_cnt(I_SEGINT, NCT), cd_cnt(I_SEGINT, NDC), &
                        c_class(I_SEGINT, NCT, numclass), cd_class(I_SEGINT, NDC, numclass), &
                        c_tot(I_SEGINT, NCT), cd_tot(I_SEGINT, NDC), t_class(I_SEGINT, numclass), &
    ...
    READ(RSI)TPOUT,TPTRIB,TPDTRIB,TPWD,TPPR,TPIN,TP_SEDSOD_PO4,PFLUXIN,TNOUT,TNTRIB,TNDTRIB,TNWD,TNPR,TNIN,TN_SEDSOD_NH4,NFLUXIN    ! TP_SEDBURIAL,TN_SEDBURIAL,
        ...
        IF (RESTART_IN) THEN
            DO JW = 1, NWB
            IF (SCREEN_OUTPUT(JW)) CALL SCREEN_UPDATE()
            ENDDO
        ENDIF
    ...
    integer(c_int) function CE_QUAL_W2_PrepareForStep() bind(C, name='CE_QUAL_W2_PrepareForStep')
        ...
        if (.NOT. END_RUN .AND. .NOT. STOP_PUSHED) then
            ...
            DO JB = BS(JW), BE(JW)
                ...
                IF (UP_FLOW(JB)) THEN
                    IF (.NOT. INTERNAL_FLOW(JB)) THEN
                        ...
                        IF (QIND(JB) + QINSUM(JB) .GT. 0.0) THEN
                            ...
                            CIN(CN(1:NAC), JB) = MAX((CINSUM(CN(1:NAC), JB)*QINSUM(JB) + &
                                                        CIND(CN(1:NAC), JB)*QIND(JB))/(QIND(JB) + QINSUM(JB)), 0.0)
                            T1(K, IU - 1) = TIN(JB)
                            T2(K, IU - 1) = TIN(JB)
                ...
                DO I = IUT, IDT - 1
                    ...
                    DO K = KT + 1, KBMIN(I)
                        ...
                        IF (.NOT. IMPLICIT_VISC(JW)) ST(K, I) = ST(K, I) + AZ(K - 1, I)*(BR(K - 1, I) + BR(K, I))* &
                                                                0.5D0*(U(K - 1, I) - U(K, I))/((AVH2(K - 1, I) &
                                                                                                + AVH2(K - 1, I + 1))*0.5D0)
                    END DO
                    GC2 = 0.0
                    ...
                    ELSE
                        ...
                        IF (KBMIN(I) /= KB(I)) THEN
                            SB(KBMIN(I), I) = GC2*(BR(KBMIN(I), I) - BR(KBMIN(I) + 1, I) + &
                                                    2.0D0*H2(K, I))*U(KBMIN(I), I)*DABS(U(KBMIN(I), I))
                        ELSE
                ...
                DO I = IU, ID - 1
                    DO K = KT, KBMIN(I)
                        IF (AX(JW) >= 0.0) THEN
                        DM(K, I) = AX(JW)*(BH2(K, I + 1)*(U(K, I + 1) - U(K, I))/DLX(I + 1) &
                                            - BH2(K, I)*(U(K, I) - U(K, I - 1))/DLX(I))/DLXR(I)
                        ELSE
                        DM(K, I) = ABS(U(K, I))*ABS(AX(JW))*H(K, JW)*(BH2(K, I + 1)* &
(U(K, I + 1) - U(K, I))/DLX(I + 1) - BH2(K, I)*(U(K, I) - U(K, I - 1))/DLX(I))/DLXR(I)     ! SW 8/2/2017 SCALE AX WITH U, FOR EXAMPLE AX=0.1U
                        ENDIF
                    END DO
                END DO
                ...
                DO I = IU, ID - 1
                    DO K = KT, KB(I) - 1
                        ...
                        ADMZ(K, I) = (BR(K, I) + BR(K + 1, I))*0.5D0* &
                                    (W(K, I + 1) + W(K, I))*0.5D0*(AB*U(K, I) + (1.0 - AB)*U(K + 1, I))
                    END DO
                END DO
                ...
                DO I = IU, ID - 1
                    ...
                    END DO
                    DO K = KT, KB(I)
                        D(I) = D(I) + (U(K, I)*BHR2(K, I) - U(K, I - 1)* &
                                    BHR2(K, I - 1) - QSS(K, I) + (UXBR(K, I) - UXBR(K, I - 1))*DLT)
                ...
                IF (DN_FLOW(JB)) THEN
                    DO K = KT, KB(ID)
                        D(ID) = D(ID) - U(K, ID - 1)*BHR2(K, ID - 1) - &
                                QSS(K, ID) + (UXBR(K, ID) - UXBR(K, ID - 1))*DLT + QOUT(K, JB)
                    END DO
                END IF
                IF (UP_HEAD(JB)) THEN
                    DO K = KT, KBMIN(IU - 1)
                ...
                IF (DN_HEAD(JB)) THEN
                    ...
                    END DO
                    DO K = KT, KB(ID)
                        D(ID) = D(ID) + (U(K, ID)*BHR2(K, ID) - U(K, ID - 1)*BHR2(K, ID - 1) - &
                                        QSS(K, ID)) + (UXBR(K, ID) - UXBR(K, ID - 1))*DLT
            ...
            DO JB = BS(JW), BE(JW)
                ...
                IF (UH_INTERNAL(JB)) THEN
                    Z(IU - 1) = ((-EL(KTWB(JWUH(JB)), UHS(JB)) + Z(UHS(JB))*COSA(JBUH(JB))) + EL(KT, IU - 1) + &
                                SINA(JB)*DLXR(IU - 1))/COSA(JB)
                ...
                DO I = IU, ID   !CONCURRENT(I=IU:ID)   !FORALL(I=IU:ID)                       !DO I=IU,ID
                    ...
                    V(I) = RHO(KT, I)*G*COSA(JB)*DLT*DLT*(BHRHO(I)*0.5D0/DLXR(I) + BHRHO(I - 1)* &
                ...
                IF (UP_FLOW(JB) .AND. HEAD_FLOW(JB)) Z(IU - 1) = (-EL(KTWB(JWUH(JB)), UHS(JB)) + &
                                                                    Z(UHS(JB))*COSA(JBUH(JB)) + EL(KT, IU - 1) &
                ...
                IF (.NOT. TRAPEZOIDAL(JW)) THEN                                                                                !SW 07/16/04
                    DO I = IU - 1, ID + 1
                        ...
                        DO WHILE (EL(KT, I) - Z(I)*COSA(JB) > EL(KTI(I), I) .AND. KTI(I) /= 2)
                            Z(I) = (EL(KT, I) - EL(KTI(I), I) - (EL(KT, I) - EL(KTI(I), I) - Z(I)* &
                                                                COSA(JB))*(B(KTI(I), I)/B(KTI(I) - 1, I)))/COSA(JB)
                        ...
                        DO WHILE (EL(KT, I) - Z(I)*COSA(JB) < EL(KTI(I) + 1, I) .AND. KTI(I) < KB(I))                   ! sw 7/18/11
                            Z(I) = (EL(KT, I) - EL(KTI(I) + 1, I) - (EL(KT, I) - EL(KTI(I) + 1, I) - &
                                                                    Z(I)*COSA(JB))*(B(KTI(I), I)/B(KTI(I) + 1, I)))/COSA(JB)
                            KTI(I) = KTI(I) + 1
...
BHR1(KT, I) = BH1(KT, I) + (BH1(KT, I + 1) - BH1(KT, I))/(0.5*(DLX(I) + DLX(I + 1)))*0.5*DLX(I)                          !SW 07/29/04
                ...
                DO I = IU, ID - 1
                    DO K = KT, KBMIN(I)
                        U(K, I) = (BHR2(K, I)*U(K, I))/BHR1(K, I) + (DLT*(-SB(K, I) + ST(K, I) - ADMZ(K, I) + ADMZ(K - 1, I) - &
                                                ADMX(K, I) + DM(K, I) - HPG(K, I) + GRAV(K, I) + UXBR(K, I)/H2(K, I)))/BHR1(K, I)
                ...
                IF (IMPLICIT_VISC(JW)) THEN
                    ...
                    DO I = IUT, IDT - 1
                        ...
                        AT(K, I) = -DLT/BHR1(K, I)*AZ(K - 1, I)*(BHR1(K - 1, I)/AVHR(K - 1, I) + &
                                                                    BR(K, I))/(AVH1(K - 1, I) + AVH1(K - 1, I + 1))
            ...
            DO JB = BS(JW), BE(JW)
                DO I = CUS(JB), DS(JB)
                    ...
                    IF (DLTADD(JW) == '      ON' .and. ABS(H1(KT, I) - H2(KT, I))/H2(KT, I) > 0.5) THEN
                        ...
                        'Computational warning |h1-h2|/h2>0.25 on Julian day = ', JDAY, &
                        ' at segment ', I, ' timestep DLT= ', DLT, '   Water surface deviation [Z,m] = ', Z(I), &
                        ' H1 layer thickness(m) = ', H1(KT, I), ' H2 layer thickenss(m)=', h2(kt, i), ' Iteration[NIT]=', &
                        nit, ' DLT reduced'
                        WARNING_OPEN = .TRUE.
                        IF (H1(KT, I) > 0.0) THEN
                        CURMAX = DLT*0.5
                    ...
                    ELSEIF (H1(KT, I) < 0.0) THEN
                        WRITE (WRN, '(A,F0.3,A,I0/4(A,F0.3))') 'Computational warning at Julian day = ', JDAY, &
                        ' at segment ', I, 'timestep = ', DLT, ' water surface deviation [Z] = ', &
                        Z(I), ' m  layer thickness = ', H1(KT, I), ' m'
                        WARNING_OPEN = .TRUE.
                        IF (DLT > DLTMIN) THEN
                        WRITE (WRN, '(A,I0/2(A,F0.3),A,I0)') 'Negative surface layer thickness in segment ', I &
                            , '  time step reduced to ', DLTMIN, ' s on day ', JDAY, ' at iteration ', NIT
                        WARNING_OPEN = .TRUE.
                        CURMAX = DLTMIN
                        GO TO 220
                        ELSE
                        WRITE (W2ERR, '(A,F0.3/A,I0)') 'Unstable water surface elevation on day ', JDAY, &
                            'negative surface layer thickness '//'using minimum timestep at iteration ', NIT
                        ...
                        DO II = MAX(CUS(JB), I - 3), MIN(DS(JB), I + 3)
                            WRITE (W2ERR, '(T4,I3,T19,F10.2,t37,f10.2,1x,f10.2,2x,f10.2)') II, H1(KT, II), QC(II), &
                                U(KT, II), ELWS(II)                           ! SW 7/13/10
                        END DO
                        TEXT = 'Runtime error - see w2.err'
                        ERROR_OPEN = .TRUE.
                        CE_QUAL_W2_Performstep = 1
                        return
                        END IF
                    END IF
                END DO
...
IF (CELERITY_LIMIT(JW)) CELRTY = SQRT((ABS(RHO(KB(I), I) - RHO(KT, I)))/1000.0*G*DEPTHB(KBI(I), I)*0.5)               ! SW 1/23/06
                    DO K = KT, KB(I)
                        ...
                        QTOT(K, I) = (ABS(U(K, I))*BHR1(K, I) + ABS(U(K, I - 1))*BHR1(K, I - 1) + &
                                    (ABS(W(K, I))*BB(K, I) + ABS(W(K - 1, I))*BB(K - 1, I))*DLX(I) &
...
220          CONTINUE
            ...
            DLT = DLTFF*CURMAX
            IF (DLT <= DLTMIN) THEN
                WRITE (WRN, '(A,F0.3/A,F0.3,A)') 'Computational warning at Julian day = ', JDAY, &
            ...
            END IF
            END DO
        END if    ! END OF MAIN DO WHILE LOOP
    ...
    integer(c_int) function CE_QUAL_W2_Dispose() bind(C, name='CE_QUAL_W2_Dispose')
        ...
        CE_QUAL_W2_Dispose = 0
    end function CE_QUAL_W2_Dispose
end module CE_QUAL_W2_Model
...
PROGRAM CE_QUAL_W2
    ...
    INTEGER       :: RESULT
    REAL          :: DEPTH
    logical       :: STOP_PUSHED, RESTART_PUSHED
    ...
    IF (RESTART_IN) THEN
        ...
        if (envirpc == '      ON') THEN
            ...
            allocate (cc_e(NCT), c_int_v(NCT), c_top(NCT), cd_e(NDC), cd_int(NDC), cd_top(NDC), &
                    c_avg(NCT), cd_avg(NDC), cn_e(NCT), cdn_e(NDC))
            cc_e = '   '
            c_int_v = 0.0
            c_top = 0.0
            cd_e = '   '
            ...
            NACD_E = 0
            open (CONE, file='w2_envirprf.npt', status='old')
            READ (CONE, '(//I1,7X,I8,5x,a3,f8.0,f8.0,9(i8,i8))') I_SEGINT, numclass, selectivec, sjday1, sjday2, istart(1), &
            iend(1), (istart(I), iend(I), I=2, I_SEGINT)
            IF (I_SEGINT == 0) I_SEGINT = 1
        Read (CONE, '(//8x,3(5x,a3,f8.3,f8.3))') VEL_VPR, VEL_INT, VEL_TOP, TEMP_VPR, TEMP_INT, TEMP_TOP, depth_vpr, d_int, d_top
            ...
            allocate (c_cnt(I_SEGINT, NCT), cd_cnt(I_SEGINT, NDC), c_class(I_SEGINT, NCT, numclass), &
                    cd_class(I_SEGINT, NDC, numclass), c_tot(I_SEGINT, NCT), cd_tot(I_SEGINT, NDC), &
                    t_class(I_SEGINT, numclass), v_class(I_SEGINT, numclass), c_sum(NCT), cd_sum(NDC))
            allocate (conc_c(NCT, numclass), conc_cd(NDC, numclass))
            allocate (d_class(I_SEGINT, numclass))
            ALLOCATE (D_TOT(I_SEGINT), D_CNT(I_SEGINT), T_TOT(I_SEGINT), T_CNT(I_SEGINT))
            ALLOCATE (V_TOT(I_SEGINT), V_CNT(I_SEGINT), VOLGL(I_SEGINT), SUMVOLT(I_SEGINT))
        ...
        DO JW = 1, NWB
            ...
            IF (UP_FLOW(JB)) THEN
                IF (.NOT. INTERNAL_FLOW(JB)) THEN
                    DO K = KT, KB(IU)
                        IF (QIND(JB) + QINSUM(JB) .GT. 0.0) THEN
                        TIN(JB) = (TINSUM(JB)*QINSUM(JB) + TIND(JB)* &
                                    QIND(JB))/(QIND(JB) + QINSUM(JB))
                        CIN(CN(1:NAC), JB) = MAX((CINSUM(CN(1:NAC), JB)* &
                                                    QINSUM(JB) + CIND(CN(1:NAC), JB)*QIND(JB))/(QIND(JB) + QINSUM(JB)), &
                                                    0.0)
                        T1(K, IU - 1) = TIN(JB)
                        T2(K, IU - 1) = TIN(JB)
                        C1S(K, IU - 1, CN(1:NAC)) = CIN(CN(1:NAC), JB)
            ...
            DO I = IUT, IDT - 1
                ...
                DO K = KT + 1, KBMIN(I)
                    ST(K, I) = WSHX(I)*DECAY(K - 1, I)*BR(K, I)
                    IF (.NOT. IMPLICIT_VISC(JW)) ST(K, I) = ST(K, I) + AZ(K - 1, I)*(BR(K - 1, I) + BR(K, I))* &
                                                        0.5D0*(U(K - 1, I) - U(K, I))/((AVH2(K - 1, I) + AVH2(K - 1, I + 1))*0.5D0)
                END DO
                GC2 = 0.0
                IF (FRIC(I) /= 0.0) GC2 = G/(FRIC(I)*FRIC(I))
            ...
            DM(K, I) = AX(JW)*(BH2(K, I + 1)*(U(K, I + 1) - U(K, I))/DLX(I + 1) - BH2(K, I)*(U(K, I) - U(K, I - 1))/DLX(I))/DLXR(I)
                    ELSE
                        DM(K, I) = ABS(U(K, I))*ABS(AX(JW))*H(K, JW)*(BH2(K, I + 1)*(U(K, I + 1) - &
    U(K, I))/DLX(I + 1) - BH2(K, I)*(U(K, I) - U(K, I - 1))/DLX(I))/DLXR(I)     ! SW 8/2/2017 SCALE AX WITH U, FOR EXAMPLE AX=0.1U
                    ENDIF
                END DO
            END DO
...
BHR1(KT, I) = BH1(KT, I) + (BH1(KT, I + 1) - BH1(KT, I))/(0.5*(DLX(I) + DLX(I + 1)))*0.5*DLX(I)                          !SW 07/29/04
            ...
            DO I = IU, ID - 1
                DO K = KT, KBMIN(I)
                    U(K, I) = (BHR2(K, I)*U(K, I))/BHR1(K, I) + (DLT*(-SB(K, I) + ST(K, I) - ADMZ(K, I) + &
                                ADMZ(K - 1, I) - ADMX(K, I) + DM(K, I) - HPG(K, I) + GRAV(K, I) + UXBR(K, I)/H2(K, I)))/BHR1(K, I)
                    IF (INTERNAL_WEIR(K, I)) U(K, I) = 0.0D0
                END DO
            END DO
            ...
            DO I = CUS(JB), DS(JB)
                ...
                IF (DLTADD(JW) == '      ON' .and. ABS(H1(KT, I) - H2(KT, I))/H2(KT, I) > 0.5) THEN
                    WRITE (WRN, '(A,F0.3,A,I0,A,F0.3/3(A,F0.3),a,i10,a)') &
                        ...
                        '   Water surface deviation [Z,m] = ', Z(I), ' H1 layer thickness(m) = ', H1(KT, I), &
                        ' H2 layer thickenss(m)=', h2(kt, i), ' Iteration[NIT]=', nit, ' DLT reduced'
                    WARNING_OPEN = .TRUE.
                    IF (H1(KT, I) > 0.0) THEN
                        CURMAX = DLT*0.5
                    ELSE
                ...
                ELSEIF (H1(KT, I) < 0.0) THEN
                    ...
                    ELSE
                        WRITE (W2ERR, '(A,F0.3/A,I0)') 'Unstable water surface elevation on day ', JDAY, &
                        'negative surface layer thickness '// &
...
220       CONTINUE
        IF (CURMAX < DLT .AND. DLT > DLTMIN) THEN
            DLT = DLTFF*CURMAX
            IF (DLT <= DLTMIN) THEN
            WRITE (WRN, '(A,F0.3/A,F0.3,A)') 'Computational warning at Julian day = ', JDAY, ' timestep = ', DLT &
                , ' sec: DLT<DLTMIN set DLT=DLTMIN'
            WARNING_OPEN = .TRUE.
            DLT = DLTMIN
            END IF
            NV = NV + 1
