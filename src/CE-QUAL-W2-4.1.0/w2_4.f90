    ...
    IF (RESTART_IN) THEN
        ...
        if (envirpc == '      ON') THEN
            ...
            READ (CONE, '(//I1,7X,I8,5x,a3,f8.0,f8.0,9(i8,i8))') I_SEGINT, numclass, selectivec, sjday1, sjday2, istart(1), iend(1), &
            ...
            allocate (c_cnt(I_SEGINT, NCT), cd_cnt(I_SEGINT, NDC), &
                ...
                c_tot(I_SEGINT, NCT), cd_tot(I_SEGINT, NDC), t_class(I_SEGINT, numclass), v_class(I_SEGINT, numclass), c_sum(NCT), cd_sum(NDC))
            ...
            DO JB = BS(JW), BE(JW)
                DO I = CUS(JB), DS(JB)
                    ...
                    IF (DLTADD(JW) == '      ON' .and. ABS(H1(KT, I) - H2(KT, I))/H2(KT, I) > 0.5) THEN
                        WRITE (WRN, '(A,F0.3,A,I0,A,F0.3/3(A,F0.3),a,i10,a)') 'Computational warning |h1-h2|/h2>0.25 on Julian day = ', &
                            JDAY, &
                            ' at segment ', I, ' timestep DLT= ', DLT, '   Water surface deviation [Z,m] = ', Z(I), &
                            ' H1 layer thickness(m) = ', H1(KT, I), ' H2 layer thickenss(m)=', h2(kt, i), ' Iteration[NIT]=', nit, ' DLT reduced'
                        WARNING_OPEN = .TRUE.
                        IF (H1(KT, I) > 0.0) THEN
                            CURMAX = DLT*0.5
                        ELSE
                    ...
                    ELSEIF (H1(KT, I) < 0.0) THEN
                        WRITE (WRN, '(A,F0.3,A,I0/4(A,F0.3))') 'Computational warning at Julian day = ', JDAY, &
                            ' at segment ', I, 'timestep = ', DLT, ' water surface deviation [Z] = ', Z(I), ' m  layer thickness = ', H1(KT, I), ' m'
                        WARNING_OPEN = .TRUE.
                        IF (DLT > DLTMIN) THEN
...
220     CONTINUE
    ...
    IF (RESTART_IN) THEN
        ...
        if(envirpc == '      ON')THEN
            ...
            allocate(cc_e(NCT),c_int_v(NCT),c_top(NCT),cd_e(NDC),cd_int(NDC),cd_top(NDC),c_avg(NCT),cd_avg(NDC),cn_e(NCT),cdn_e(NDC))
            cc_e='   '
            c_int_v=0.0
            c_top=0.0
            cd_e='   '
