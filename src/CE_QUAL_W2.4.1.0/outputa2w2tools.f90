      ...
      IF (JDAY .GE. NXTBIO) THEN
         ...
         DO J = 1, NIBIO
            ...
            DO K = KTWB(JW), KBI(I)
               ...
               END DO
        WRITE(BIOEXPFN(J),'(F8.2,",",I8,",",3(F8.2,","),"<NZOOE-NZOOS+1>"(F8.3,","),I8,",",2(F8.2,","),A,",",I0,",",I0)') &
        JDAY,IBIO(J),DEPTHM(K,I),T1(K,I),GAMMA(K,I),&          ! CB 1/6/17
         ...
         DO J = 1, NIKTSR
            ...
            IF (K >= KTWB(JW)) THEN      ! SW 4/4/2018 ADDED TO ELIMINATE THE LAST VALUE OF A VARIABLE BEING USED FOR CASE WHEN ESTR IS NEGATIVE
               ...
               IF (ICE_COMPUTATION) THEN
                  IF (SEDIMENT_CALC(JW)) THEN
            WRITE (TSR(J),'(f10.3,",",18(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),T1(K,I),U(K,I), &
            QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),    &     ! SW 8/13/06  
            BI(KTWB(JW), I), SHADE(I), ICETH(I), TVOLAVG, rn(i), rs(i), ranlw(jw), rb(i), re(i), rc(i), &
            (ADJUSTR(C2CH(JAC)), JAC=1, NAC), &  ! CB 7/26/07
                        (ADJUSTR(EPCH(JE)), JE=1, NEP), (ADJUSTR(MACCH(JM)), JM=1, NMC), SEDCH, SEDPCH, SEDNCH, SEDCCH, &
            (ADJUSTR(CDCH(JAD)),JAD=1,NACD(JW)),(ADJUSTR(KFCH(JF)),JF=1,NAF(JW)),(ADJUSTR(APCH(JA)),JA=1,NAL), &
            (ADJUSTR(ANCH(JA)),JA=1,NAL),(ADJUSTR(ALCH(JA)),JA=1,NAL)    ! SW 10/20/15 
                  ELSE
            WRITE (TSR(J),'(f10.3,",",18(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),T1(K,I),U(K,I), &
            QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),      &     ! SW 8/13/06  
            BI(KTWB(JW), I), SHADE(I), ICETH(I), TVOLAVG, rn(i), rs(i), ranlw(jw), rb(i), re(i), rc(i), &
            ...
            (ADJUSTR(ANCH(JA)),JA=1,NAL),(ADJUSTR(ALCH(JA)),JA=1,NAL)    ! SW 10/20/15  
               ...
               ELSE
                  IF (SEDIMENT_CALC(JW)) THEN
            WRITE (TSR(J),'(f10.3,",",17(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),T1(K,I),U(K,I), &
            QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),    &     ! SW 8/13/06  
            BI(KTWB(JW),I),SHADE(I),TVOLAVG,rn(i),rs(i),ranlw(jw),rb(i),re(i),rc(i), &
            (ADJUSTR(C2CH(JAC)),JAC=1,NAC),(ADJUSTR(EPCH(JE)),            &  ! CB 7/26/07
            JE=1, NEP), (ADJUSTR(MACCH(JM)), JM=1, NMC), SEDCH, SEDPCH, SEDNCH, SEDCCH, &
            (ADJUSTR(CDCH(JAD)),JAD=1,NACD(JW)),(ADJUSTR(KFCH(JF)),JF=1,NAF(JW)), &
            (ADJUSTR(APCH(JA)),JA=1,NAL),(ADJUSTR(ANCH(JA)),JA=1,NAL),(ADJUSTR(ALCH(JA)),JA=1,NAL)    ! SW 10/20/15  
                  ELSE
            WRITE (TSR(J),'(f10.3,",",17(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),T1(K,I),U(K,I),&
            QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),      &      ! SW 8/13/06  
            BI(KTWB(JW),I),SHADE(I),TVOLAVG,rn(i),rs(i),ranlw(jw),rb(i),re(i),rc(i), &
            ...
            ELSE      ! SW 4/4/2018
               IF (ICE_COMPUTATION) THEN
                  IF (SEDIMENT_CALC(JW)) THEN
            WRITE (TSR(J),'(f10.3,",",18(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),-99.,-99.,QC(I), &
            SRON(JW)*1.06,-99.,DEPTHB(KB(I),I),    &     ! SW 8/13/06  
...
WRITE (PRF(JW), '(A8,I4/(8(E13.6,2X)))') ADJUSTL(CNAME2(CN(JC))), NRS, (C2(K, IPRF(JP, JW), CN(JC))*CMULT(CN(JC)), &     ! CB 1/24/05
  ...
  WRITE (PRF(JW), '(A8,I4/(8(E13.6,2X)))') ADJUSTL(CDNAME2(CDN(JD, JW))), NRS, (CD(K, IPRF(JP, JW), CDN(JD, JW)) &      ! CB 1/24/05
            ...
            elseif (jw == 1) then   ! write out individual files on these days
               ...
               IF (CONSTITUENTS) THEN
             WRITE(NUNIT,'(1000(A,","))')'Seg#','ElevWaterSurf(m)','Q(m3/s)','SurfaceTemp(oC)','Depth(m)','Width(m)', &
             'VolWeighTemp(oC)' ,(CNAME2(CN(JC)),JC=1,NAC),(CDNAME2(CDN(JD,JW)),JD=1,NACD(JW))
               ELSE
 WRITE (NUNIT, '(1000(A,","))') 'Seg#', 'ElevWaterSurf(m)', 'Q(m3/s)', 'SurfaceTemp(oC)', 'Depth(m)', 'Width(m)', 'VolWeighTemp(oC)'
               ENDIF
                  ...
                  DO JB = BS(JJ), BE(JJ)
                     DO I = CUS(JB), DS(JB)
                        ...
                        IF (CONSTITUENTS) THEN
                             ...
                             TVOLAVG,(C2(K,I,CN(JAC))*CMULT(CN(JAC)),JAC=1,NAC), &
                             (CD(K,I,CDN(JD,JW))*CDMULT(CDN(JD,JW)),JD=1,NACD(JW))
                        ELSE
                       WRITE (NUNIT, '(I5,",",100(F12.3,","))') I, ELWS(I), QC(I), T2(K, I), DEPTHB(KB(I), I), B(KTI(I), I), TVOLAVG
                        ENDIF
                     ENDDO
                  ENDDO
               ENDDO
         ...
         IF (JDAY >= NXTMSP(JW) .OR. JDAY >= SPRD(SPRDP(JW) + 1, JW)) THEN
            ...
            IF (SPRC(JW) == '     ONV') THEN    ! SW 8/31/2018
               DO J = 1, NISPR(JW)
                  ...
                  VOLTOT = 0.0
                  DO KK = KTWB(JW), KB(J)
                     VOLTOT = VOLTOT + VOL(KK, J)
                     TVOLAVG = TVOLAVG + T1(KK, J)*VOL(KK, J)
                  ENDDO
                  IF (KB(J) >= KTWB(JW)) THEN
                     tvolavg = tvolavg/voltot
                  ELSE
                     TVOLAVG = -99.0
                  ENDIF
                  WRITE (CONV1(K, J), '(F10.3)') TVOLAVG
               ENDDO
               WRITE (SPRV(JW), '(A,",",F10.3,",",1000(A,","))') 'Temperature', JDAY, (CONV1(K, J), J=1, NISPR(JW))
               ...
               DO JC = 1, NAC
                  IF (PRINT_CONST(CN(JC), JW)) THEN
                     DO J = 1, NISPR(JW)
                        ...
                        DO KK = KTWB(JW), KB(J)
                           VOLTOT = VOLTOT + VOL(KK, J)
                           TVOLAVG = TVOLAVG + C2(KK, ISPR(J, JW), CN(JC))*CMULT(CN(JC))*VOL(KK, J)
                        ENDDO
                        IF (KB(J) >= KTWB(JW)) THEN
                           tvolavg = tvolavg/voltot
                        ELSE
                           TVOLAVG = -99.0
                        ENDIF
                        WRITE (CONV1(K, J), '(F10.3)') TVOLAVG                                                ! SW 8/31/18
                     END DO
                     WRITE (SPRV(JW), '(A38,",",F10.3,",",1000(A,","))') CNAME3(CN(JC)), JDAY, (CONV1(K, J), J=1, NISPR(JW))
                  END IF
               END DO
               IF (CONSTITUENTS) THEN
                  DO JD = 1, NACD(JW)
                     IF (PRINT_DERIVED(CDN(JD, JW), JW)) THEN
                        DO J = 1, NISPR(JW)
                           ...
                           DO KK = KTWB(JW), KB(J)
                              VOLTOT = VOLTOT + VOL(KK, J)
                              TVOLAVG = TVOLAVG + CD(KK, ISPR(J, JW), CDN(JD, JW))*CDMULT(CDN(JD, JW))*VOL(KK, J)
                           ENDDO
                           IF (KB(J) >= KTWB(JW)) THEN
                              tvolavg = tvolavg/voltot
                           ELSE
                           ...
                           WRITE (CONV1(K, J), '(F10.3)') TVOLAVG                                     ! SW 8/31/18
                        END DO
                       WRITE (SPRV(JW), '(A38,",",F10.3,",",1000(A,","))') CDNAME3(CDN(JD, JW)), JDAY, (CONV1(K, J), J=1, NISPR(JW))
                     END IF
                  END DO
               END IF
...
9864           FORMAT('ZONE T="', f9.3, '"', ' I=', I3, ' J=', I3, ' F=POINT')
               DO JB = BS(JW), BE(JW)
                  DO I = CUS(JB), DS(JB) + 1
                     ...
                     IF (I /= DS(JB) + 1) THEN
                        IF (HABTATC == '      ON') THEN
                  WRITE (CPL(JW),9999) X1(I),ELWS(I),U(K,I),-W(K,I),T1(K,I),RHO(K,I),HAB(K,I),(C2(K,I,CN(JC)), &
                  JC=1,NAC),(CD(K,I,CDN(JD,JW)),JD=1,NACD(JW))   ! SW 1/17/17
                        ELSE
                  WRITE (CPL(JW),9999) X1(I),ELWS(I),U(K,I),-W(K,I),T1(K,I),RHO(K,I),(C2(K,I,CN(JC)),JC=1,NAC), &
                  (CD(K,I,CDN(JD,JW)),JD=1,NACD(JW))            ! SW 1/17/17
                        ENDIF
...
WRITE (CPL(JW), 9999) X1(I), ELWS(I), XDUM, XDUM, XDUM, XDUM, (XDUM, JJ=1, NAC), (XDUM, JJ=1, NACD(JW))          ! SW 1/17/17 7/15/14
                     ...
                     DO K = KTWB(JW), KMX - 1
                        ...
                        IF (HABTATC == '      ON') THEN
                  WRITE (CPL(JW),9999) X1(I),ELWS(I)-DEPTHM(K,I),U(K,I),-W(K,I),T1(K,I),RHO(K,I),HAB(K,I), &
                  (C2(K,I,CN(JC)),JC=1,NAC),(CD(K,I,CDN(JD,JW)),JD=1,NACD(JW))   ! SW 1/17/17 
                        ELSE
                  ...
                  (CD(K,I,CDN(JD,JW)),JD=1,NACD(JW))   ! SW 1/17/17 
                        ...
                        IF (K == KB(I)) THEN
                           IF (HABTATC == '      ON') THEN
                    WRITE (CPL(JW),9999) X1(I),ELWS(I)-DEPTHB(K,I),U(K,I), -W(K,I),T1(K,I),RHO(K,I),HAB(K,I), &
                    (C2(K,I,CN(JC)),JC=1,NAC),(CD(K,I,CDN(JD,JW)),JD=1,NACD(JW))   ! SW 1/17/17   
                           ELSE
                    WRITE (CPL(JW),9999) X1(I),ELWS(I)-DEPTHB(K,I),U(K,I), -W(K,I),T1(K,I),RHO(K,I),(C2(K,I,CN(JC)), &
                    JC=1,NAC),(CD(K,I,CDN(JD,JW)),JD=1,NACD(JW))   ! SW 1/17/17   
                           ENDIF
                ...
                WRITE (WDO2(JFILE, 4), '(F10.3,",",1000(F10.4,","))') JDAY, (CDAVGW(JWD, CDN(JD, JWUSP(JS))), JD=1, NACD(JWUSP(JS)))
                  ...
                  ELSE
                   ...
                   WRITE (WDO2(JFILE, 3), '(F10.3,",",1000(F10.4,","))') JDAY, (CAVG(JSSS(JBUSP(JS)), JBUSP(JS), CN(JC)), JC=1, NAC)
                     ...
                     IF (DERIVED_CALC) then
                cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qsp(js)*cdavg(jsss(jbusp(js)), &
                jbusp(js),cdn(1:nacd(jwusp(js)),jwusp(js)))    ! cb 1/16/13
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JSSS(JBUSP(JS)), &
                JBUSP(JS),CDN(JD,JWUSP(JS))),JD=1,NACD(JWUSP(JS)))  
                     end if
                  ENDIF
               ENDIF
            ENDDO
                ...
                WRITE (WDO2(JFILE, 4), '(F10.3,",",1000(F10.4,","))') JDAY, (CDAVGW(JWD, CDN(JD, JWUPU(JS))), JD=1, NACD(JWUPU(JS)))
                  ...
                  ELSE
                   ...
                   WRITE (WDO2(JFILE, 3), '(F10.3,",",1000(F10.4,","))') JDAY, (CAVG(JSSS(JBUPU(JS)), JBUPU(JS), CN(JC)), JC=1, NAC)
                     ...
                     IF (DERIVED_CALC) then
                        if (pumpon(js)) then   ! cb 1/16/13
                  cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qpu(js)*cdavg(jsss(jbupu(js)), &
                  jbupu(js),cdn(1:nacd(jwupu(js)),jwupu(js))) 
                        end if
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JSSS(JBUPU(JS)),JBUPU(JS), &
                CDN(JD,JWUPU(JS))),JD=1,NACD(JWUPU(JS)))  
                     end if
                  ENDIF
               ENDIF
            ENDDO
...
cdwdo(cdn(1:nacd(jw), jw), j) = cdwdo(cdn(1:nacd(jw), jw), j) + qpi(js)*cdavgw(jwd, cdn(1:nacd(jwupi(js)), jwupi(js)))     ! cb 1/16/13
                  ...
                  ELSE
                   ...
                   WRITE (WDO2(JFILE, 3), '(F10.3,",",1000(F10.4,","))') JDAY, (CAVG(JSSS(JBUPI(JS)), JBUPI(JS), CN(JC)), JC=1, NAC)
                     ...
                     IF (DERIVED_CALC) then
                cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qpi(js)*cdavg(jsss(jbupi(js)), &
                jbupi(js),cdn(1:nacd(jwupi(js)),jwupi(js)))     ! cb 1/16/13
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JSSS(JBUPI(JS)),JBUPI(JS), &
                CDN(JD,JWUPI(JS))),JD=1,NACD(JWUPI(JS)))  
                     end if
                  ENDIF
               ENDIF
            ENDDO
                ...
                WRITE (WDO2(JFILE, 4), '(F10.3,",",1000(F10.4,","))') JDAY, (CDAVGW(JWD, CDN(JD, JWUGT(JS))), JD=1, NACD(JWUGT(JS)))
                  ...
                  ELSE
                   ...
                   WRITE (WDO2(JFILE, 3), '(F10.3,",",1000(F10.4,","))') JDAY, (CAVG(JSSS(JBUGT(JS)), JBUGT(JS), CN(JC)), JC=1, NAC)
                     ...
                     IF (DERIVED_CALC) then
                cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qgt(js)*cdavg(jsss(jbugt(js)), &
                jbugt(js),cdn(1:nacd(jwugt(js)),jwugt(js)))     ! cb 1/16/13
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JSSS(JBUGT(JS)),JBUGT(JS), &
                CDN(JD,JWUGT(JS))),JD=1,NACD(JWUGT(JS)))  
                     end if
                  ENDIF
               ENDIF
            ENDDO
