         ...
         IF(MASS_BALANCE(JW).AND.CONTOUR(JW).AND.DERIVED_CALC)THEN     ! TO COMPUTE TP AND TN INFLOWS AND OUTFLOWS FOR MASSBAL.OPT FILE
                ...
                IF (JB == JBTR(JT)) THEN
                  ...
                  DO K=KTTR(JT),KBTR(JT)
                    ...
                    IF (QTR(JT) < 0.0) THEN
                        ...
                        ENDDO
                      TPOUT = TPOUT-(TPALG+TPBOD+TPZ+PO4(K,I)+LDOMP(K,I)+&
                      LPOMP(K,I)+RDOMP(K,I)+RPOMP(K,I))*QTR(JT)*QTRF(K,JT)*DLT/1000.
                      TNOUT = TNOUT-(TNALG+TNBOD+TNZ+NO3(K,I)+NH4(K,I)+LDOMN(K,I)+&
                      LPOMN(K,I)+RDOMN(K,I)+RPOMN(K,I))*QTR(JT)*QTRF(K,JT)*DLT/1000.
                    ELSE
                        DO JC=NAS,NAE
                        ...
                        ENDDO
                      TPTRIB(JW) = TPTRIB(JW)+ (TPALG+TPBOD+TPZ+CTR(NPO4,JT)+CTR(NLDOMP,JT)+&
                      CTR(NRDOMP,JT)+CTR(NLPOMP,JT)+CTR(NRPOMP,JT))*QTR(JT)*QTRF(K,JT)*DLT/1000.
                      TNTRIB(JW) = TNTRIB(JW)+ (TNALG+TNBOD+TNZ+CTR(NNH4,JT)+CTR(NNO3,JT)+&
                      CTR(NLDOMN,JT)+CTR(NRDOMN,JT)+CTR(NLPOMN,JT)+CTR(NRPOMN,JT))*QTR(JT)*QTRF(K,JT)*DLT/1000.
                    END IF
                  END DO
                END IF
              END DO
            ...
            IF (DIST_TRIBS(JB)) THEN
              DO I=IU,ID
                ...
                IF (QDT(I) < 0.0) THEN
                      ...
                      TNOUT = TNOUT-(TNALG+TNBOD+TNZ+NO3(KT,I)+NH4(KT,I)+LDOMN(KT,I)+LPOMN(KT,I)+&
                      RDOMN(KT,I)+RPOMN(KT,I))*QDT(I)*DLT/1000.
                ELSE
                        DO JC=NAS,NAE
                        ...
                        ENDDO
                      TPDTRIB(JW) = TPDTRIB(JW)+ (TPALG+TPBOD+TPZ+CDTR(NPO4,JB)+CDTR(NLDOMP,JB)+&
                      CDTR(NRDOMP,JB)+CDTR(NLPOMP,JB)+CDTR(NRPOMP,JB))*QDT(I)*DLT/1000.
                      TNDTRIB(JW) = TNDTRIB(JW)+ (TNALG+TNBOD+TNZ+CDTR(NNH4,JB)+CDTR(NNO3,JB)+&
                      CDTR(NLDOMN,JB)+CDTR(NRDOMN,JB)+CDTR(NLPOMN,JB)+CDTR(NRPOMN,JB))*QDT(I)*DLT/1000.
                END IF
              END DO
            END IF
            IF (WITHDRAWALS) THEN
                ...
                IF (QWD(JWD) /= 0.0) THEN
                  IF (JB == JBWD(JWD)) THEN
                    ...
                    DO K=KTW(JWD),KBW(JWD)      !CONCURRENT(K=KTW(JWD):KBW(JWD))                 ! FORALL
                        ...
                        ENDDO
                      TPWD(JW) = TPWD(JW) + (TPALG+TPBOD+TPZ+C1S(K,I,NPO4)+C1S(K,I,NLDOMP)+C1S(K,I,NRDOMP)+&
                      C1S(K,I,NLPOMP)+C1S(K,I,NRPOMP))*QSW(K,JWD)*DLT/1000.
                      TNWD(JW) = TNWD(JW) + (TNALG+TNBOD+TNZ+C1S(K,I,NNH4)+C1S(K,I,NNO3)+C1S(K,I,NLDOMN)+&
                      C1S(K,I,NRDOMN)+C1S(K,I,NLPOMN)+C1S(K,I,NRPOMN))*QSW(K,JWD)*DLT/1000.
                    END DO
                  END IF
                END IF
              END DO
            ...
            IF (PRECIPITATION(JW)) THEN
              DO I=IU,ID    !CONCURRENT (I=IU:ID)                                  !FORALL
                TPALG=0.0;TNALG=0.0;TPZ=0.0;TNZ=0.0;TPBOD=0.0;TNBOD=0.0
                        ...
                        ENDDO
                      TPPR(JW) = TPPR(JW) + (TPALG+TPBOD+TPZ+CPR(NPO4,JB)+CPR(NLDOMP,JB)+CPR(NRDOMP,JB)+CPR(NLPOMP,JB)+&
                      CPR(NRPOMP,JB))*QPR(I)*DLT/1000.
                      TNPR(JW) = TNPR(JW) + (TNALG+TNBOD+TNZ+CPR(NNH4,JB)+CPR(NNO3,JB)+CPR(NLDOMN,JB)+CPR(NRDOMN,JB)+&
                      CPR(NLPOMN,JB)+CPR(NRPOMN,JB))*QPR(I)*DLT/1000.
              END DO
            END IF
            IF (UP_FLOW(JB)) THEN
              DO K=KT,KB(IU)
                IF (.NOT. HEAD_FLOW(JB)) THEN
                        ...
                        ENDDO
                      TPIN(JW) = TPIN(JW) + (TPALG+TPBOD+TPZ+CIN(NPO4,JB)+CIN(NLDOMP,JB)+&
                      CIN(NRDOMP,JB)+CIN(NLPOMP,JB)+CIN(NRPOMP,JB))*QINF(K,JB)*QIN(JB)*DLT/1000.
                      TNIN(JW) = TNIN(JW) + (TNALG+TNBOD+TNZ+CIN(NNH4,JB)+CIN(NNO3,JB)+CIN(NLDOMN,JB)+&
                      CIN(NRDOMN,JB)+CIN(NLPOMN,JB)+CIN(NRPOMN,JB))*QINF(K,JB)*QIN(JB)*DLT/1000.
            ...
            IF (DN_FLOW(JB)) THEN
                DO K=KT,KB(ID)
                        ...
                        ENDDO
                      TPOUT(JW) = TPOUT(JW) + (TPALG+TPBOD+TPZ+C1S(K,ID,NPO4)+C1S(K,ID,NLDOMP)+&
                      C1S(K,ID,NRDOMP)+C1S(K,ID,NLPOMP)+C1S(K,ID,NRPOMP))*QOUT(K,JB)*DLT/1000.   ! C1S(KT:KB(ID),ID,JC)
                      TNOUT(JW) = TNOUT(JW) + (TNALG+TNBOD+TNZ+C1S(K,ID,NNH4)+C1S(K,ID,NNO3)+&
                      C1S(K,ID,NLDOMN)+C1S(K,ID,NRDOMN)+C1S(K,ID,NLPOMN)+C1S(K,ID,NRPOMN))*QOUT(K,JB)*DLT/1000.
                ENDDO
            ENDIF
...
!**** Kinetic fluxes
    ...
    if(sediment_diagenesis)then
      ...
      ApplyBubbTurb .AND. BUBBLES_CALCULATION)Call CEMABubblesReleaseTurbulence
