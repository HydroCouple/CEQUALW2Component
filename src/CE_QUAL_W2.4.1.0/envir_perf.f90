...
subroutine envirp
   ...
   USE GLOBAL; USE MAIN; use NAMESC; use screenc, only: nit, jday; use tvdc, only: constituents 
   use rstart, only: eltm; use GEOMC, only: depthb
   USE ENVIRPMOD
   IMPLICIT NONE
   CHARACTER(1) :: I_INT
   INTEGER :: N
...
!  initializing variables at first call
   if (NIT == 1 .or. iopenfish == 0) then
      ...
      READ (CONE, 1050) (CC_E(JC), C_INT_V(JC), C_TOP(JC), JC=1, NCT)
...
! Constituents
                  IF (CONSTITUENTS) THEN
                     do jc = 1, nac_e
                        ...
                        do jj = 2, numclass
                           if (c2(k, i, jac)*cmult(jac) .lt. c_crit .and. c2(k, i, jac)*cmult(jac) .ge. c_crit - c_int_v(jac)) then
                              c_class(N, jc, jj) = c_class(N, jc, jj) + dltt*vol(k, i)
                              go to 220
                           else
                              c_crit = c_crit - c_int_v(jac)
                           end if
                           if (jj .eq. numclass .and. c2(k, i, jac)*cmult(jac) .lt. c_crit + c_int_v(jac)) then
                              c_class(N, jc, jj) = c_class(N, jc, jj) + dltt*vol(k, i)
                           end if
                        end do
            ...
            do jc = 1, nac_e
               ...
               do i = 1, numclass
                  if (i .eq. 1) then
                     ...
                     conc_c(jc, i) = conc_c(jc, i - 1) - c_int_v(cn_e(jc))
                  end if
...
4001           format("<nacd_e>" (a8, '_interval, Fraction_of_volume,'))
      ...
      DEallocate (cc_e, c_int_v, c_top, cd_e, cd_int, cd_top, c_avg, cd_avg, cn_e, cdn_e)
