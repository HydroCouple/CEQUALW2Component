...
Module Fishy
      ...
      REAL, ALLOCATABLE, DIMENSION (:)   :: v_tot,v_cnt, d_tot, d_cnt, t_tot,&
                                            t_cnt, d_avg, t_avg, v_avg, d_sum, t_sum, v_sum, sumvolt  ! SW 4/30/2018
      REAL, ALLOCATABLE, DIMENSION (:,:) :: v_class, t_class, d_class
...
! initial fish output
    write(INITIALFN,'(a291)')'Part#,Seg#,XLocationwithinSegmentfromUpstreamSide(m), &
    &Layer#,VerticalDistfromTop(m),LateralDistfromLeftBank,Branch#, &
    &ParticleInModel(=0),JDAYleftsystem,DetentionTime(days),RemovalMechanism,SedVelocity(m/d),DateStart'
    do jf=1,nfish     
    write(INITIALFN,'(i7,",",1x,10(f10.3,","),f8.4,",",f12.4)')jf,fishes(jf,1),fishes(jf,2),&
    fishes(jf,3),fishes(jf,4),fishes(jf,5),fishes(jf,6),fishes(jf,12),fishes(jf,14),&
...
!Check for Boundary Violations: Lateral Direction Check (1 of 2)
  ...
  if(fyvel.eq.0.0)then   ! SW 2/01/01 logic for removing particles laterally also
        ...
        ELSE IF(FYLOC.GT.B(FKMP,FIMP).and.fyvel.gt.0.0.AND.RIMPBR(FIMP,1)>0) THEN   
            WRITE(DIAGFN,*) 'LateralRIGHT: Particle move to new branch on JDAY:',&
            jday,'FYVEL=',fyvel,'OLD I:',FIMP,'NEW I:', RIMPBR(FIMP,1),' OLD branch:',&
            FNBP,' NEW branch:',RIMPBR(FIMP,2)
            FIMP=RIMPBR(FIMP,1)
        ...
        ELSE IF(FYLOC.LT.0.0.and.fyvel.Lt.0.0.AND.LIMPBR(FIMP,1)>0) THEN  
            WRITE(DIAGFN,*) 'LateralLEFT: Particle move to new branch on JDAY:',&
            jday,'FYVEL=',fyvel,'OLD I:',FIMP,'NEW I:', LIMPBR(FIMP,1),&
            ' OLD branch:',FNBP,' NEW branch:',LIMPBR(FIMP,2)
            FIMP=LIMPBR(FIMP,1)
            FYLOC= B(FKMP,FIMP)*0.5   ! PLACE IN MIDDLE LATERALLY
        ...
        ELSEIF (FYLOC.GT.B(FKMP,FIMP).and.RIMPBR(FIMP,1)==0.0) THEN   ! reflect 
            IF(HITSTICKSIDE)THEN
                ...
                fishes(fn,12)=1.0                ! FISH LEAVES SYSTEM
                fishes(fn,16)=7.0                ! Particle leaves by hitting side wall and sticking-RHS
                WRITE(DIAGFN,*) 'Hit Stick RHSide: Particle sticks to side wall on JDAY:',&
                jday, 'Data:',fishes(fn,:)
            ELSE
        ...
        ELSEIF (FYLOC.LT.0.0 .and.LIMPBR(FIMP,1)==0) THEN  ! reflect 
            IF(HITSTICKSIDE)THEN
                ...
                fishes(fn,12)=1.0                ! FISH LEAVES SYSTEM
                fishes(fn,16)=6.0                ! Particle leaves by hitting side wall and sticking-LHS
                WRITE(DIAGFN,*) 'Hit Stick LHSide: Particle sticks to side wall on JDAY:',&
                jday, 'Data:',fishes(fn,:)
            ELSE
           FYLOC = B(FKMP,FIMP)*YREFL 
           ENDIF
...
!Check for Boundary Violations: Horizontal Direction
   ...
   10   IF (FXLOC.GT.DLX(FIMP)) THEN
           ...
           ELSEIF(FXVEL(5) > 0.0 .AND. DHS(FNBP)==0)THEN
              ! CHECK IF A BRANCH DOWNSTREAM TO PASS THE PARTICLE TO
               IF(NEXT_BRANCH(FNBP))THEN
                WRITE(DIAGFN,*) 'At end of branch: Particle moves to next branch on JDAY:',&
                jday,'OLD I:',FIMP,'NEW I:',FNEXT(FNBP),'Data:',fishes(fn,:)
                FIMP=FNEXT(FNBP)
                FYLOC= B(FKMP,FIMP)*0.5   ! PLACE IN MIDDLE LATERALLY
           ...
           GOTO 20
           ELSEIF(FXVEL(5) > 0.0 .AND. DHS(FNBP) > 0)THEN
                WRITE(DIAGFN,*) 'DHS: Particle move to new branch on JDAY:',&
                jday,'FXVEL=',fxvel(5),'OLD I:',FIMP,'NEW I:', DHS(FNBP),' OLD branch:',FNBP
                FIMP=DHS(FNBP)
                FYLOC= B(FKMP,FIMP)*0.5   ! PLACE IN MIDDLE LATERALLY
        ...
        ELSE IF (FXLOC.LT.0) THEN
          ...
          IF (FIMP.EQ.CUS(FNBP) .AND. UHS(FNBP)==0) THEN      ! The UpStream end of Branch JBP or UNBP
              ...
              ELSEIF(FIMP.EQ.CUS(FNBP) .AND. UHS(FNBP)==-1 .AND. FXVEL(5) < 0.0) THEN 
                ...
                WRITE(DIAGFN,*) 'At External Upstream Head Boundary: Particle Leaves System on JDAY:',&
                jday, 'Data:',fishes(fn,:)
           GOTO 20
              ELSEIF(FIMP.EQ.CUS(FNBP) .AND. UHS(FNBP)==-1 .AND. FXVEL(5) >= 0.0) THEN 
             FXLOC = DLX(FIMP)*XREFL    ! REflect partilce off UPSTREAM boundary 
              ELSEIF(FIMP.EQ.CUS(FNBP) .AND. UHS(FNBP)>0 .AND. FXVEL(5) >= 0.0) THEN     
                    FXLOC = DLX(FIMP)*XREFL    ! REflect partilce off UPSTREAM boundary   
              ELSEIF(FIMP.EQ.CUS(FNBP) .AND. UHS(FNBP)>0 .AND. FXVEL(5) < 0.0) THEN     
                WRITE(DIAGFN,*) 'UHS: Particle move to new branch on JDAY:',&
                jday,'FXVEL=',fxvel(5),'OLD I:',FIMP,'NEW I:', DHS(FNBP),' OLD branch:',FNBP
                FIMP=UHS(FNBP)
                FYLOC= B(FKMP,FIMP)*0.5   ! PLACE IN MIDDLE LATERALLY
                FXLOC= DLX(FIMP)*0.5      ! PLACE IN MIDDLE OF SEGMENT                           
            ...
            FXLOC = DLX(FIMP) + FXLOC                          !    move to
              ELSE
            WRITE(DIAGFN,*)'***NO RESOLUTION OF FXLOC<0*** JDAY,I,K,BR#,FN#,FYLOC,FXLOC:',&
               ...
               if(kti(fimp).eq.KTWBF)then
                  if(fzloc.lt.z(fimp))then
                   ...
                   if(fzloc.le.h(kti(fimp),fjr)+z(fimp))then
                     if(z(fimp).gt.0.0)write(DATADEBUGFN,*)'Debug Error in vertical reflection:FN,I,K,JDAY',&
                     fn,fimp,fkmp,jday
...
!Check for Boundary Violations: Vertical Direction Check (2 of 2)
   ...
   30   IF (FZLOC >= H(FKMP,FJR)) THEN                     ! Did fish move down below current layer?  -- YES
          ...
          IF (FKMP+1.GT.KBI(FIMP)) THEN                 ! Did fish move below bottom layer? -- YES   USE KBI RATHER THAN KB SW 8/2018
                ...
                WRITE(DIAGFN,*) 'Particle Hit and Stick Bottom: Particle Leaves System on JDAY:',&
                jday, 'Data:',fishes(fn,:)
                  ELSE
                    FZLOC = H(FKMP,FJR)*(1-ZBOTREFL)               !    Logic for rebounding off of the bottom
                  ENDIF
                  ...
                  IF (K.LT.KTWB(WB)) THEN                               ! WATER used for value-blanking cells above 9952 surface in TecPlot
                    IF (SHOWSKY) THEN                                  ! Show Colors of Day Above Water Surface
                      ...
                      ELSE IF ((MILTIME.GT.SKYDUSK).AND.&
                        ...
                        WATER = -25                                    !   ORANGE
                      ELSE                                             ! CHECK
                        WRITE(DATADEBUGFN,*) 'ERROR: MILTIME not Corresponding &
                        &with Intervals Set for Determining Sky Color for TecPlot'
                        STOP
                      END IF
                    ELSE
...
!
                  ...
                  ELSE                              !Both Top Nodes exist
                    ...
                    HVERT(2) = -0.5*H(K-1,WB)
                  END IF
                  IF (K.GT.KBI(I)) THEN              !Both Bottom Nodes are below Water Body Bottom   USE KBI SW 8/2018
                  ...
                  ELSE IF (K+1.GT.KBI(I)) THEN       !Only Bottom-most Node below Water Body Bottom    USE KBI SW 8/2018
                    UVERT(3) = U(K,I-1)
                    UVERT(4) = 0.0                    ! Helps satisfy no-slip condition on bottom (i.e., U=0)
                    HVERT(3) = 0.5*H(K,WB)
...
! final fish output
    write(FINALFN,'(A215)')'Part#,&
    &Seg#,XLocationwithinSegmentfromUpstreamSide(m),Layer#,VerticalDistfromTop(m),&
    &LateralDistfromLeftBank,Branch#,ParticleInModel(=0),&
    &JDAYleftsystem,DetentionTime(days),RemovalMechanism,SedVelocity(m/d),DateStart'
    do jf=1,nfish     
    write(FINALFN,'(i7,",",1x,10(f10.3,","),f8.4,",",f12.4)')jf,&
    fishes(jf,1),fishes(jf,2),fishes(jf,3),fishes(jf,4),fishes(jf,5),&
    fishes(jf,6),fishes(jf,12),fishes(jf,14),fishes(jf,15),fishes(jf,16),sedvel(jf),delaydate(jf)
    end do
...
! DATE:  Date particles are deposited
    ...
    Allocate(ifish(nfishseg),ifisht(nfishseg),ifishb(nfishseg),delaydateinit(nfishseg),&
    fxlocinit(nfishseg),fzlocinit(nfishseg),sedvelinit(nfishseg)) 
        READ(DIAGFN,*)
        DO I=1,NFISHSEG
