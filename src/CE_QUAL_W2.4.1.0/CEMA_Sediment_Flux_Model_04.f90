...
!        SD_PO40     =po4zz/1000.0
   ...
   Do While (.true.)
      ...
      SD_NH3toNO3 = SD_Ae_NH3_NO3**2.0*SD_Theta_NH3_NO3**(SD_T(1) - 20.) &
                    /SD_s*SD_Ae_HS_NH4_Nit/(SD_Ae_HS_NH4_Nit + SD_NH3(1))*SD_O20/(2.0*SD_Ae_HS_O2_Nit + SD_O20)  ! [m/d] = [m2/d2] / [m/d] * [-] * [-]
   ...
   !Layer 1
   VolWater = CellArea(SegNumI)*SD_H1
   Call CEMADisGasPhaseDistribution(SD1_Sulfide, MW_Constituent, &
                                    SedTemp1, HenryConst_H2S, VolWater, SulfideG_SD1, SulfideD_SD1)
   !Layer 2
   VolWater = CellArea(SegNumI)*SD_H2
   Call CEMADisGasPhaseDistribution(SD2_Sulfide, MW_Constituent, &
                                    SedTemp2, HenryConst_H2S, VolWater, SulfideG_SD2, SulfideD_SD2)
   ...
   Dissolved_NH3_Src = Dissolved_NH3_Src + &
                       (SD1_Ammonium*PW_RelRate1 + SD2_Ammonium*PW_RelRate2)/(CellThickness*CellArea(SegNumI))  !gm/m�/d
   ...
   Dissolved_H2S_Src = Dissolved_H2S_Src + &
                       (SD1_SulfiMinus*PW_RelRate1 + SD2_SulfiMinus*PW_RelRate2)/(CellThickness*CellArea(SegNumI))  !gm/m�/d
   ...
   If (SD_O20 > SD_Ox_Threshol) Then !Only if aerobic conditions exist
      Do GenBODNum = 1, NumGenBODConstituents
         Do iTemp = 1, 1 !Only Aerobic layer consumption and decay
            ...
            SedGenBODDecayRate(GenBODNum, SegNumI) = SedGenBODConsRate(GenBODNum, SegNumI)* &
                                                     SedGenBODConsTCoeff(GenBODNum, SegNumI)**(SD_Tw - 20.)
            SedGenBODConc(GenBODNum, SegNumI, iTemp) = (1 - SedGenBODDecayRate(GenBODNum, SegNumI)* &
                                                        SD_tc)*SedGenBODConc(GenBODNum, SegNumI, iTemp)
   ...
   Do GenBODNum = 1, NumGenBODConstituents
      !Aerobic Layer
      SedGenBODConc(GenBODNum, SegNumI, 1) = SedGenBODConc(GenBODNum, SegNumI, 1) + &
                                             (SD_BODD(GenBODNum, 1) + Dissolved_BOD_Src(GenBODNum, 1))/SD_H1*SD_tc
      !Anaerobic Layer
      SedGenBODConc(GenBODNum, SegNumI, 2) = SedGenBODConc(GenBODNum, SegNumI, 2) + &
                                             (SD_BODD(GenBODNum, 2) + Dissolved_BOD_Src(GenBODNum, 2))/SD_H2*SD_tc
   End Do !GenBODNum
...
!**** pH, carbon dioxide, bicarbonate, and carbonate concentrations
   ...
   c_bottom = (c2(kb(SegNumI), SegNumI, NLPOM) + &
               c2(kb(SegNumI), SegNumI, NRPOM))*dexp(poms(jw)*h(kb(SegNumI), jw)/DZ(kb(SegNumI) - 1, SegNumI))
