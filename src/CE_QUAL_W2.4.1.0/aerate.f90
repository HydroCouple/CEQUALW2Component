  ...
  SUBROUTINE AERATE
     ...
     IF (NAER .EQ. 0) NAER = 1
ALLOCATE (DZMULTA(NAER), IASEG(NAER), KTOPA(NAER), KBOTA(NAER), SMASS(NAER), ATIMON(NAER), ATIMOFF(NAER), DOOFF(NAER), DOON(NAER), &
               IPRB(NAER), KPRB(NAER), ACTUAL_MASS(NAER), AERATEO2(NAER), CUMDOMASS(NAER))
     DZMULTA = 0.0; ACTUAL_MASS = 0.0; CUMDOMASS = 0.0
     READ (AERATEFN, 1013)
1013 FORMAT(/)
     DO I = 1, NAER
        READ (AERATEFN, '(I8,I8,I8,F8.0,F8.0,F8.0,3F8.0,2I8)') IASEG(I), KTOPA(I), KBOTA(I), SMASS(I), ATIMON(I), ATIMOFF(I), &
           DZMULTA(I), DOOFF(I), DOON(I), IPRB(I), KPRB(I)
     END DO
...
106     JDAY1 = 0.0
     ELSE
        ...
        WRITE (AERATEFN, '(A114)') ' &
           JDAY,INSTMASSRATE#1(KGO2/D),CUMMASS#1(KGO2),DOPROBE#1(MG/L),INSTMASSRATE#2(KGO2/D),CUMMASS#2(KGO2),DOPROBE#2(MG/L)'
     ENDIF
...
!Fix this
     WRITE(AERATEFN,'(F9.3,"<NAER>"(1X,E12.3,1X,E12.3,1X,F8.3))') &
     JDAY,(ACTUAL_MASS(II),CUMDOMASS(II),O2(KPRB(II),IPRB(II)),II=1,NAER)
