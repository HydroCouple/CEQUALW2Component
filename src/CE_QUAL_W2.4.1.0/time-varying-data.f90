...
! Type declaration
   ...
   REAL(R8), ALLOCATABLE, TARGET, DIMENSION(:)    :: QDTRO, TDTRO, ELUHO, ELDHO, QWDO, QTRO, TTRO, QINO, TINO
   ...
   INTEGER, ALLOCATABLE, DIMENSION(:)     :: PRT, UHT, DHT, INC, DTC, PRC, UHC, DHC, &
                                             OTQ, MET, EXT, ODYNS
   LOGICAL, ALLOCATABLE, DIMENSION(:)     :: DYNPUMPF
   LOGICAL, ALLOCATABLE, DIMENSION(:)     :: INFLOW_CONST, TRIB_CONST, DTRIB_CONST, PRECIP_CONST, OTQF, TRCF, &
                                             DTCF, INCF, PRCF, METF, DYNEF, TRQF, TRTF, DTTF, DTQF, INQF, INTF, PRQF, PRTF, EXTF
...
! Allocation declarations
  ...
  ALLOCATE (CTRNX(NCT, NTR), CINNX(NCT, NBR), QOUTNX(KMX, NBR), CDTRNX(NCT, NBR), CPRNX(NCT, NBR), TUHNX(KMX, NBR), TDHNX(KMX, NBR))
   ...
   ALLOCATE (INFLOW_CONST(NBR), TRIB_CONST(NTR), DTRIB_CONST(NBR), &
    PRECIP_CONST(NBR), OTQF(NBR), TRCF(NTR), DTCF(NBR), INCF(NBR), TRQF(NTR), TRTF(NTR), DTTF(NBR), DTQF(NBR), INQF(NBR), INTF(NBR))
   ...
   IF (INFORMAT == '$') THEN
      ...
      DO I = 1, IMX
         ...
         IF (SHADEI(I) < 0.0) THEN
            BACKSPACE (SHD)
            READ (SHD, *) J, SHADEI(I), TTLB(I), TTRB(I), CLLB(I), CLRB(I), SRLB1(I), SRLB2(I), &
               SRRB1(I), SRRB2(I), (TOPO(I, J), J=1, IANG), SRFJD1(I), SRFJD2(I)
         ENDIF
      ENDDO
   ELSE
...
!** Downstream head
   ...
   DEALLOCATE (TUHNX, TDHNX, QSTRNX, CUHO, CDHO, CUHNX, CDHNX, INCF, DTCF, PRCF, METF, &
               ODYNS, NXDYNS, NXESTRT, DYNEF, JJS, PRTF, PRQF)
   DEALLOCATE (EUHF, TUHF, CUHF, EDHF, TDHF, CDHF, XX)
   DEALLOCATE (INFLOW_CONST, TRIB_CONST, DTRIB_CONST, PRECIP_CONST, PUMPD, NXPUMP, EPU2, EONPU2, EOFFPU2, QPU2, OTQF, TRCF, &
               TRQF, TRTF, DTTF, DTQF, INQF, INTF)
   RETURN
