...
SUBROUTINE WITHDRAWAL
  USE GLOBAL; USE GEOMC; USE TVDC; USE SELWC; USE LOGICC; USE MAIN, ONLY: DERIVED_CALC,&
  CDN,tdgon, jsg,  nnsg, ndo, jwd, NGN2, NGCTDG, EA   ! cb 1/16/13
  ...
  IF(DERIVED_CALC)then                    ! cb 1/16/13
    ...
    !if(tdgon)then                  ! cb 11/6/17
      ...
      IF(ngctdg /= 0)THEN
          ...
          cdavg(js,jb,NDC)  = 100.*(0.79*(cavg(js,jb,NGN2)/n2sat) + 0.21*(cavg(js,jb,ndo)/dosat))
      ENDIF
    !end if
  end if
ELSE
    ...
    IF(DERIVED_CALC)then
      ...
      !if(tdgon)then                ! cb 11/6/17
        ...
        IF(ngctdg /= 0)THEN
          ...
          cdavgw(jwd,NDC)  = 100.*(0.79*(cavgw(jwd,NGN2)/n2sat) + 0.21*(cavgw(jwd,ndo)/dosat))
        ENDIF
      !end if
    end if
  ELSE
...
!***********************************************************************************************************************************
  ...
  ALLOCATE (TCNELEV(NUMTEMPC),TCJB(NUMTEMPC),TCJS(NUMTEMPC), TCELEV(NUMTEMPC,11),TCTEMP(NUMTEMPC),&
  TCTEND(NUMTEMPC),TCTSRT(NUMTEMPC),NCOUNTC(NST,NBR),TCISEG(NUMTEMPC),TCKLAY(NUMTEMPC),TCELEVCON(NUMTEMPC)) 
  ALLOCATE (TCYEARLY(NUMTEMPC), JBMON(NUMTEMPC),JSMON(NUMTEMPC),TCNTR(NUMTEMPC)) 
  ALLOCATE (VOLM(NWB),MONCTR(NUMTEMPC),NCOUNTCW(NWD),QWDFRAC(NWD),QSTRFRAC(NST,NBR),DYNSEL(NUMTEMPC),&
  SELD(NUMTEMPC),NXSEL(NUMTEMPC),TEMP2(NUMTEMPC))       
      ...
      NCOUNTC=0
      DO J=1,NUMTEMPC
      READ(NUNIT,'(8X,A8,I8,I8,A8,F8.0,F8.0,F8.0,I8,10(F8.0))')TCNTR(J),TCJB(J),TCJS(J),&
      TCYEARLY(J),TCTSRT(J),TCTEND(J),TCTEMP(J),TCNELEV(J),(TCELEV(J,N),N=1,TCNELEV(J))
        IF(TCNTR(J)=='      ST')THEN      
      ...
      READ(NUNIT,'(8X,A8,I8,A8,F8.0,F8.0,F8.0,I8,2I8,A8)')TSPLTCNTR(J),TSPLTJB(J),TSYEARLY(J),TSTSRT(J),&
      TSTEND(J),TSPLTT(J),NOUTS(J),(JSTSPLTT(J,N),N=1,2),ELCONTSPL(J)
  ...
  ENDIF   
      ...
      IF (JDAY.GE.NXTSTR) THEN
        ...
        DO JB=1,NBR
            IF(NSTR(JB) > 0)THEN
            IFILE=IFILE+1
            WRITE (IFILE,'(F10.4,"<NSTR(JB)>"F10.2,"<NSTR(JB)>"F10.2,"<NSTR(JB)>"F10.2)') JDAY,(TAVG(I,JB),I=1,NSTR(JB)),&
            (QSTR(I,JB),I=1,NSTR(JB)),(ESTR(I,JB),I=1,NSTR(JB))
            END IF
         ENDDO          
          IF(NWD > 0)THEN
            IFILE=IFILE+1
...
14    JDAY1=0.0
  ...
  read (NUNIT,'(/)')
  do j=1,numtempc
    read(NUNIT,'(8x,a8,i8,i8,a8,f8.0,f8.0,f8.0,i8,10(f8.0))')tcntr(j),tcjb(j),tcjs(j),tcyearly(j),tctsrt(j),&
    tctend(j),tctemp(j),tcnelev(j),(tcelev(j,n),n=1,tcnelev(j))
    if(tcntr(j)=='      ST')then
      tcelev(j,tcnelev(j)+1)=ESTR(tcjs(j),tcjb(j))   ! always put the original elevation as the last elevation
    else
      tcelev(j,tcnelev(j)+1)=EWD(tcjs(j))   ! always put the original elevation as the last elevation
            ...
            do n=1,nouts(j)
              do nj=1,nouts(jj)
                if (jstsplt(j,n) == jstsplt(jj,nj)) then
                  write (w2err, '(A,I0,A)') 'w2_selective.npt USGS ERROR-- Split outlet number ', jstsplt(j,n), &
    ...
    do j=1,numtsplt
      do n=1,nouts(j)
        if (tsprior(j,n) < -1) then
          write (w2err, '(A,I0,A,I0,A)') 'w2_selective.npt USGS ERROR-- Priority input for outlet ', &
          jstsplt(j,n), ' in group ', j, ' is less than -1.'
          ERROR_OPEN = .TRUE.            ! will trigger the program to end when this subroutine is completed
        end if
        if (tsminhead(j,n) > 0.0 .and. tsmaxhead(j,n) > 0.0 .and. tsminhead(j,n) > tsmaxhead(j,n)) then
          write (wrn, '(A,I0,A,I0,A)') 'w2_selective.npt USGS WARNING-- Minimum and maximum head constraints for outlet ', &
          jstsplt(j,n), ' in group ', j, ' are such that the outlet cannot ever be used.'
          WARNING_OPEN = .TRUE.
        end if
            ...
            if (tspltcntr(j) .eq. tcntr(jj) .and. (tspltcntr(j) .eq. '      WD' .or. tspltjb(j) == tcjb(jj))) then
              do n=1,nouts(j)
                if (jstsplt(j,n) == tcjs(jj)) then
                  write (w2err, '(A,I0,A)') 'w2_selective.npt USGS ERROR-- Outlet number ',tcjs(jj),&
        ...
        if ((tctsrt(jj) >= tctsrt(j) .and. tctsrt(jj) <  tctend(j)) .or.                                                           &
          ...
          if (tcntr(j) .eq. tcntr(jj) .and. (tcntr(j) .eq. '      WD' .or. tcjb(j) == tcjb(jj))) then
            if (tcjs(j) == tcjs(jj)) then
              write (w2err, '(A,I0,A)') 'w2_selective.npt USGS ERROR-- Tower outlet number ', tcjs(j),&
...
! Output some results.
  if (jday.ge.nxtstr) then
    ...
    do jb=1,nbr
      if (nstr(jb) > 0) then
        ifile=ifile+1
        write (ifile,'(f10.4,"<nstr(jb)>"f10.2,"<nstr(jb)>"f10.2,"<nstr(jb)>"f10.2)') jday,(tavg(i,jb),i=1,&
        nstr(jb)),(qstr(i,jb),i=1,nstr(jb)),(estr(i,jb),i=1,nstr(jb))
      end if
    end do
    if (nwd > 0) then
      ifile=ifile+1
