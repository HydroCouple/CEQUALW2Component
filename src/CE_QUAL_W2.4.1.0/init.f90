SUBROUTINE INIT
   ...
   USE STRUCTURES; USE TRANS; USE TVDC; USE SELWC; USE GDAYC
   USE SCREENC; 
   USE TDGAS; USE RSTART
...
! Logical controls
   ...
   IF (CONSTITUENTS) THEN
      ...
      IF(CGQ10(JG) == 0.0 .AND. CG0DK(JG)==-1.0 .AND. CG1DK(JG)==0.0 .AND. CGS(JG)==0.0 .AND. &
         CGLDK(JG)==0.0 .AND. CGKLF(JG)==0.0 .AND. CGCS(JG)==0.0)THEN
...
! Time and printout control variables
   ...
   TSRD(NTSR + 1:NOD) = TMEND + 1.0; WDOD(NWDO + 1:NOD) = TMEND + 1.0
   RSOD(NRSO + 1:NOD) = TMEND + 1.0; DLTD(NDLT + 1:NOD) = TMEND + 1.0
...
! Hydraulic structures
   ...
   ALLOCATE (ESTR(NST,NBR),WSTR(NST,NBR),QSTR(NST,NBR),KTSW(NST,NBR),KBSW(NST,NBR), &
